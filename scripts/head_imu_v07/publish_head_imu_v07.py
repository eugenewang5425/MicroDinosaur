...
for name,label in labels.items():
 ...
 screens.append(dict(file=str(dest.relative_to(R)),label=label,source=str(src),image_sha256=sha(dest),model_sha256=H,status='ACTUAL_CAD_RENDER',scope_note='Isolation only for render; no hidden retired parts in saved model'))
for srcname,label in [('before_head_open','v06_修改前_头部外壳'),('before_carrier','v06_修改前_相机Pi载架')]:
 ...
 screens.append(dict(file=str(dest.relative_to(R)),label=label,source=str(src),image_sha256=sha(dest),model_sha256=B['source_sha256'],status='PRIOR_CAD_RENDER_NOT_CURRENT'))
...
write(C/'previews/manifest.json',dict(version='20260913_07',model_sha256=H,screenshots=[dict(file=name+'.png',label=label,image_sha256=sha(C/'previews'/(name+'.png'))) for name,label in labels.items()],old_previews_moved_to=str(archive/'previews')))
write(archive/'retired_status.json',dict(status='RETIRED_v06_REFERENCE_NOT_CURRENT_NOT_ALTERNATIVE',model_sha256=B['source_sha256'],local_parts_library='HEAD_LOCAL_v06_NOT_CURRENT.blend',current_model=str(current),current_sha256=H,old_previews=oldpreviews))
write(P/'publication.json',dict(current_model=str(current),sha256=H,backup=str(archive),screenshots=str(shots),old_screenshots_hash_verified=len(history),new_screenshots=len(screens),all_screenshots=len(history)+len(screens),fastener_schedule_before_rows=before_count,fastener_schedule_rows=len(rows),source_blueprint_sha256=V['source_blueprint_sha256'],candidate_is_not_second_current=True))
