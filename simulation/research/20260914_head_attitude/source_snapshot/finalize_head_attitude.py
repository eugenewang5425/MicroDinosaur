"""Write reviewed findings and verify final local artifacts without publishing."""
...
def main():
    ...
    followup = json.loads((OUT/'runtime_followup/audit.json').read_text())
    assert audit['status'] == followup['status'] == 'PASS'
    groups = summary['groups']
    mean = lambda policy, mode: groups[f'nominal__{policy}__straight__{mode}']['mean']
    metrics = [('相机偏离期望前方 RMS°', 'camera_heading_error_rms_deg', 1),
        ('去趋势周期偏航 RMS°', 'camera_heading_detrended_rms_deg', 1),
        ('相机偏航角速度 RMS°/s', 'camera_yaw_rate_error_rms_deg_s', 1),
        ('俯仰 RMS°', 'camera_pitch_rms_deg', 1), ('横滚 RMS°', 'camera_roll_rms_deg', 1),
        ('前进速度 cm/s', 'body_vx_mean_m_s', 100)]
    lines = ['# 头部IMU已用于头部姿态控制', '',
    ...
    for label, key, scale in metrics:
        cells = [f'{mean(policy,"off")[key]*scale:.2f} → **{mean(policy,"imu")[key]*scale:.2f}**' for policy in ('v7', 's42_no_neck')]
        lines.append(f'| {label} | '+ ' | '.join(cells)+' |')
    lines += ['', '两个策略、三个确认初态的朝向误差、周期偏航和偏航角速度共18个逐项比较，全部优于原方案和单独滤波。这里的RMS不是最大摆角，也不是持续朝一个方向转动的角速度。', '',
        ...
        '在工作区根目录复跑当前入口（输出目录必须新建）：', '', '```powershell',
    ...
    e = make_operating_experiment('v7', 'imu')
    contract = dict(head_control=asdict(e.head_config), head_joint_names=list(e.kinematics.names[1:]),
        hardware=asdict(e.sim.case), imu_transport=asdict(e.transport), actor_observation_size=81, actor_action_size=19,
        untouched_raw_policy_last_action=True, target_adapter_before_existing_filter_and_bus=True,
        user_reference='navigation desired yaw + intentional head offsets; gravity-level pitch/roll',
        stationary_yaw_alignment='body IMU tilt and four quantized neck encoders through static CAD FK',
        no_dynamic_world_truth_in_new_head_loop=True, head_translation_control=False,
        real_hardware_validated=False, hardware_control_connected=False,
        frozen_confirmation_age_s=.12, operational_max_age_s=.04,
        known_limitations=['Weak intentional roll tracking', 'Finite six-axis yaw drift',
            'Slow IMU releases stabilization', 'Legacy actor gravity uses simulation truth'])
    (OUT/'runtime_contract.json').write_text(json.dumps(contract, indent=2), encoding='utf-8')
    cli = json.loads((OUT/'runtime_cli_check/result.json').read_text())['metrics']
    followup_nominal = json.loads((OUT/'runtime_followup/v7__nominal__imu.json').read_text())
    assert cli == followup_nominal
    for policy in ('v7', 's42_no_neck'):
        video = OUT/'videos'/policy/'comparison.mp4'
        reader = imageio.get_reader(str(video))
        try:
            assert reader.count_frames() == 300 and reader.get_meta_data()['fps'] == 25
            assert reader.get_data(150).shape == (720, 1280, 3)
        finally:
            reader.close()
    extra = ['head_attitude_runtime.py', 'demo_head_attitude.py', 'verify_head_attitude_runtime.py',
        'summarize_head_attitude.py', 'audit_head_attitude.py', 'render_head_attitude.py', 'finalize_head_attitude.py']
    for name in extra:
        shutil.copyfile(ROOT/name, OUT/'source_snapshot'/name)
    (OUT/'final_audit.json').write_text(json.dumps(dict(status='PASS', formal_completed=82,
        no_falls=True, original_frozen_audit=audit['status'], runtime_followup=followup['status'],
        tests_passed=14, current_cli_matches_runtime=True, videos_validated=True,
        production_unchanged=True, extra_source_hashes={name: sha(ROOT/name) for name in extra}), indent=2), encoding='utf-8')
    manifest = {str(p.relative_to(OUT)): sha(p) for p in sorted(OUT.rglob('*')) if p.is_file() and p.name != 'artifact_manifest.json'}
    ...
    print(json.dumps(dict(status='PASS', artifacts=len(manifest), formal_simulations=82, tests=14)))
