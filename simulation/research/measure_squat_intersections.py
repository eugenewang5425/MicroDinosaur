...
sys.path.insert(0,str(Path(__file__).resolve().parent/'20260914_compact_fold/tools'))
...
for frame in json.loads((OUT/'cad_intersections.json').read_text()):
    for row in frame['new_pairs']:
        pair=tuple(row['parts'])
        if pair not in pairs and pair[::-1] not in pairs:pairs.append(pair)
result=[]
