"""Matched finite PPO runs with fixed actual LR and official normalized export."""
import os
...
from demonstration_expert import SOURCE,ROOT,OUT
...
def main():
    p=argparse.ArgumentParser();p.add_argument('--arm',choices=('legacy','archive','demonstration'),required=True)
    p.add_argument('--out',type=Path,required=True);p.add_argument('--envs',type=int,default=64)
    p.add_argument('--iterations',type=int,default=5);p.add_argument('--seed',type=int,default=43)
    ...
    cfg.agent.load_run=source.name;cfg.agent.load_checkpoint=resume.name
    cfg=replace(cfg,enable_nan_guard=True)
    previous=Path('D:/microduck_rl/logs/rsl_rl/microdinosaur_imu_owned_terrain/20260914_resume_512x100/run_provenance.json')
    sources={Path(p) for p in json.loads(previous.read_text())['source_hashes']}
    sources.update({Path(__file__),ROOT/'demonstration_cfg.py',ROOT/'demonstration_expert.py',
        ROOT/'demonstration_nan_guard.py',Path('D:/microduck_rl/src/mjlab_microduck/demonstration_ppo.py'),OUT/'RECIPE.md'})
    if a.arm=='demonstration':sources.update({OUT/'step_demonstrations.npz',OUT/'dataset.json'})
    ...
    r=dict(status='RUNNING',arm=a.arm,envs=a.envs,iterations=a.iterations,seed=a.seed,started_unix=time.time(),
        ...
        anchor_steps=2,anchor_batch_size=1024,normalization='official export required',
        actor_dim=81,action_dim=19,head_runtime='yaw owned, unchanged',
        imitation={'legacy':'512 archive + 512 old teacher on eligible on-policy states',
            'archive':'1024 archived successful old teacher pairs; no on-policy imitation',
            'demonstration':'512 archive + 512 accepted low-speed demonstrations; no on-policy imitation'}[a.arm])
