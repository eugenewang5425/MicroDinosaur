...
def test_torch_source_is_pinned_to_a_cuda_index_on_aarch64():
    ...
    # Platform-scoped entries are allowed (Windows x86_64 is CPU-only on PyPI
    # too and routes to cu128); every entry must name a PyTorch CUDA index and
    # the aarch64 one must exist.
    markers = [src["marker"] for src in sources]
    assert any("aarch64" in m for m in markers), (
        "the aarch64 torch source disappeared -> DGX Spark falls back to "
        "PyPI's CPU wheel and `train` dies with IndexError in select_gpus()."
    )
    for src in sources:
...
def test_linux_x86_64_resolution_stays_on_pypi():
    ...
    Windows x86_64 is deliberately NOT covered: PyPI's win_amd64 wheel is
    CPU-only (same trap as aarch64), so it is routed to the cu128 index.
    """
    pypi_entries = [
        p for p in _packages("torch") if _registry(p) == "https://pypi.org/simple"
    ]
    assert pypi_entries, "no PyPI torch entry left — linux x86_64 lost its wheel"
    for pkg in pypi_entries:
        # The PyPI entry must cover linux x86_64 (its marker is a disjunction
        # that includes it), and must stay a non-CUDA build.
        assert "sys_platform == 'linux'" in _markers(pkg), (
            f"the PyPI torch entry no longer covers linux: {_markers(pkg)!r}"
        )
