...
def make_bam_actuator(target_names_expr: str = r"^(?!passive_).*") -> FrictionDRBamActuatorCfg:
    """BAM actuator cfg with a custom joint-target regex.

    Used by models that add extra servos outside the BAM family (e.g. MicroDinosaur's
    SC09 tail): the XL330 BAM group excludes those joints, and they get their
    own actuator cfg instead.
    """
    return FrictionDRBamActuatorCfg(
        **{**_BAM_ACTUATOR_KWARGS, "target_names_expr": (target_names_expr,)}
    )
