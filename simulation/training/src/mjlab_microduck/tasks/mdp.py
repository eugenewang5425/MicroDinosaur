...
from mjlab.utils.lab_api.math import (
    matrix_from_quat, wrap_to_pi, quat_apply, quat_from_angle_axis,
    quat_mul, quat_from_euler_xyz, quat_error_magnitude, yaw_quat,
    quat_apply_inverse,
)
...
# ---------------------------------------------------------------------------
# Probe: print the PPO loss breakdown (MICRODINO_PROBE=lossprint).
# The mirror loss is added to the total update loss directly, not scaled by the
# reward, so its magnitude must be compared against the surrogate loss to know
# whether it dominates the gradient.  Remove once the recipe is settled.
# ---------------------------------------------------------------------------
import os as _os

if "lossprint" in _os.environ.get("MICRODINO_PROBE", ""):
    _orig_ppo_update = _PPO.update
    _probe_updates = [0]

    def _probe_update(self):
        out = _orig_ppo_update(self)
        if _probe_updates[0] % 5 == 0:
            parts = "  ".join(
                f"{k}={float(v):.6f}" for k, v in out.items() if isinstance(v, (int, float))
            )
            print(f"[probe] update {_probe_updates[0]}: {parts}", flush=True)
        _probe_updates[0] += 1
        return out

    _PPO.update = _probe_update
    print("[mdp] Probe active: printing the PPO loss breakdown")
...
_ARM_JOINT_PATTERNS = [r".*arm_l.*", r".*arm_r.*"]
...
def set_random_ground_state(
    ...
    side_left_prob: float = 0.0,
    side_right_prob: float = 0.0,
    sitting_prob: float = 0.2,
    standing_prob: float = 0.0,
    prone_z_min: float = 0.20,
    prone_z_max: float = 0.25,
    side_z_min: Optional[float] = None,
    side_z_max: Optional[float] = None,
    ...
    """Reset to a random ground state: face-down, face-up, on either side, sitting, or standing.
    ...
    Modes (probabilities are normalized; they need not sum to 1.0):
      ...
      - side-left / side-right:     ±90° ROLL (log-roll), random yaw, z in
                                    [side_z_min, side_z_max]. Which physical side
                                    is down is not load-bearing (the task is
                                    mirror-symmetric) — both buckets exist so a
                                    fall to either side is represented.
    ...
    Side buckets exist because a biped does not only fall forward or backward:
    a sideways fall (hip check, one skate losing grip) lands on the side of the
    trunk, where the recovery motion is a roll to belly/supine FIRST — a
    different skill from the pitch-family rises. Requires the groundcontact
    model (the side of the trunk must be able to rest on the floor).
    ...
    # Side spawns default to the prone z range: a 90° log-roll rests no higher
    # than the belly, and sharing the range keeps one interpenetration margin
    # for all "lying" buckets (measured: side rest z ≈ prone rest z).
    if side_z_min is None:
        side_z_min = prone_z_min
    if side_z_max is None:
        side_z_max = prone_z_max

    total = (face_down_prob + face_up_prob + side_left_prob + side_right_prob
             + sitting_prob + standing_prob)
    p_fd  = face_down_prob / total
    p_fu  = (face_down_prob + face_up_prob) / total
    p_sl  = (face_down_prob + face_up_prob + side_left_prob) / total
    p_sr  = (face_down_prob + face_up_prob + side_left_prob + side_right_prob) / total
    p_sit = (face_down_prob + face_up_prob + side_left_prob + side_right_prob
             + sitting_prob) / total
    ...
    face_down = torch.stack([ s * cy, -s * sy,  s * cy,  s * sy], dim=1)
    face_up   = torch.stack([ s * cy,  s * sy, -s * cy,  s * sy], dim=1)
    # ±90° roll about the body's long axis (x): q_yaw ⊗ q_roll(±90°).
    # Roll +90° carries body y (left) to world z (up) → the RIGHT side is down.
    side_right = torch.stack([ s * cy,  s * cy,  s * sy,  s * sy], dim=1)
    side_left  = torch.stack([ s * cy, -s * cy, -s * sy,  s * sy], dim=1)
    ...
    new_quat[is_sl]    = side_left[is_sl]
    new_quat[is_sr]    = side_right[is_sr]
    new_quat[is_sit]   = sitting[is_sit]
    new_quat[is_stand] = sitting[is_stand]

    # Random z per env: prone heights for face-down/up, a little higher for the
    # side buckets (a 90° log-roll rests on the trunk's side face, which is
    # wider than the belly/back), low for sit, ~standing for stand.
    z_prone = torch.rand(num, device=env.device) * (prone_z_max - prone_z_min) + prone_z_min
    z_side  = torch.rand(num, device=env.device) * (side_z_max - side_z_min) + side_z_min
    ...
    new_z = z_prone.clone()
    new_z = torch.where(is_side, z_side, new_z)
...
class Figure8VelocityCommand(RelativeHeadingVelocityCommand):
    """cmd[2] aims at a virtual target sliding along a figure-8 in FRONT of the robot.

    The target is parameterised in the ROBOT's frame (so it can never be
    "escaped" by drifting — the 8-shape travels with the robot and the robot
    carves it out by following), using the Gerono lemniscate:

        x_t(θ) =  R · cos(θ)
        y_t(θ) =  R · sin(θ) · cos(θ)          θ = 2π·t/period + phase

    cmd[2] = clamp(atan2(y_t, x_t), ±heading_max) — the heading error toward
    the moving target. Because the target sweeps left and right through the
    lobes, tracking it produces the classic figure-8 path.

    cmd[0] (throttle) is NOT resampled by the manager; it is driven by the same
    θ so the duck pushes on the lobes (where the curve is long) and coasts
    through the crossings:

        cmd[0] = v_min + (v_max − v_min) · (0.5 + 0.5·cos(2θ))

    so the episode covers push/coast phases in both lobes rather than a
    constant throttle. Phase is randomised per env at reset (DR: no shared
    clock between envs).

    Deployment: the runtime feeds cmd[2] from its own path planner, or the
    operator steers by hand — same interface as RelativeHeadingVelocityCommand.
    """

    def __init__(self, cfg, env: ManagerBasedRlEnv):
        super().__init__(cfg, env)
        self._env_ref = env
        self._period_s = float(getattr(cfg, "period_s", 8.0))
        self._radius = float(getattr(cfg, "radius", 0.8))
        self._v_min = float(getattr(cfg, "throttle_min", 0.0))
        self._v_max = float(getattr(cfg, "throttle_max", 0.5))
    ...
    def _resample_command(self, env_ids: torch.Tensor) -> None:
        super()._resample_command(env_ids)
        n = len(env_ids)
        # Random phase: no two envs carve the same 8 on the same clock.
        self._phase[env_ids] = torch.rand(n, device=self.device) * 2.0 * math.pi
        self._t[env_ids] = 0.0

    def _update_command(self) -> None:
        # Advance each env's clock, then aim cmd[2] at the lemniscate point.
        self._t += float(self._env_ref.step_dt)
        theta = 2.0 * math.pi * (self._t / self._period_s) + self._phase

        x_t = self._radius * torch.cos(theta)
        y_t = self._radius * torch.sin(theta) * torch.cos(theta)
        target_angle = torch.atan2(y_t, x_t)
        self.vel_command_b[:, 2] = target_angle.clamp(-self._heading_max, self._heading_max)

        # Throttle: push on the lobes, coast through the crossings.
        ramp = 0.5 + 0.5 * torch.cos(2.0 * theta)
        self.vel_command_b[:, 0] = self._v_min + (self._v_max - self._v_min) * ramp


@_dataclass(kw_only=True)
class Figure8VelocityCommandCfg(RelativeHeadingVelocityCommandCfg):
    class_type: type = Figure8VelocityCommand
    # Seconds for one full figure-8.
    period_s: float = 8.0
    # Half-width of the 8 in metres (robot-frame target scale).
    radius: float = 0.8
    # Throttle range swept by the lemniscate phase (cmd[0]).
    throttle_min: float = 0.0
    throttle_max: float = 0.5

    def build(self, env: ManagerBasedRlEnv) -> "Figure8VelocityCommand":
        return Figure8VelocityCommand(self, env)
...
def lateral_push_reward(
    env: ManagerBasedRlEnv,
    sensor_name: str,
    command_name: str,
    vel_gate_ref: float = 0.2,
    min_abduction: float = 0.08,
    asset_cfg: SceneEntityCfg = _DEFAULT_ASSET_CFG,
) -> torch.Tensor:
    """Reward the SKATING push: the swing leg abducted OUT to the side.

    The stride env's other terms (single_support, glide, air_time) reward the
    TIMING of a stride but say nothing about its DIRECTION, and a policy can
    satisfy all of them with a walk-like front-to-back swing (measured on a
    trained roller ckpt: L/R hip_pitch correlated +0.51, hip_roll asymmetric
    0.94 vs 0.52 rad). Real skating drives the blade out sideways and pushes
    through abduction, so this pays the abduction of the SWING leg:

        reward = Σ_feet swing_foot · clamp(|hip_roll − default| − min_abduction, 0)
                 · forward_gate

    - swing foot only (air_time > 0): the stance leg's abduction is posture,
      not a stroke.
    - ``min_abduction`` band: a few degrees of natural splay is free; only the
      deliberate push stroke pays.
    - forward_gate: swinging the leg out while standing still earns nothing.

    Use with a POSITIVE weight. Pairs with relaxing ``hip_roll_neutral`` (which
    otherwise fights the stroke) — see the stride_rollers env cfg.
    """
    from mjlab.sensor import ContactSensor
    sensor: ContactSensor = env.scene[sensor_name]
    air = sensor.data.current_air_time  # (num_envs, num_feet)
    assert air is not None
    swing = (air > 0.0).float()

    asset: Entity = env.scene[asset_cfg.name]
    if not hasattr(env, "_lateral_push_ids"):
        ids, _ = asset.find_joints([r"^(left|right)_hip_roll$"])
        env._lateral_push_ids = torch.tensor(ids, device=env.device)
    ids = env._lateral_push_ids
    dev = torch.abs(
        asset.data.joint_pos[:, ids] - asset.data.default_joint_pos[:, ids]
    )
    push = torch.clamp(dev - min_abduction, min=0.0)
    per_env = torch.sum(swing * push, dim=1)
...
def pitch_swing_penalty(
    env: ManagerBasedRlEnv,
    sensor_name: str,
    command_name: str,
    vel_gate_ref: float = 0.2,
    free_band: float = 0.35,
    asset_cfg: SceneEntityCfg = _DEFAULT_ASSET_CFG,
) -> torch.Tensor:
    """Penalise the WALK-LIKE front-to-back leg swing while pushing.

    The complement of ``lateral_push_reward``: skating's hip_pitch travel is
    small (the stroke is abduction + extension), while a walk-like gait swings
    the free leg through a large pitch arc. Returns the NEGATIVE of the excess
    swing (use with a POSITIVE weight, like the other *_penalty helpers):

        penalty = −Σ_feet swing_foot · clamp(|hip_pitch − default| − free_band, 0)
                  · forward_gate

    ``free_band`` is deliberately generous (0.35 rad ≈ 20°): a skating stroke
    does extend the hip, and the term must not flatten the leg entirely — it
    only discourages the big alternating pitch arc of a step.
    """
    from mjlab.sensor import ContactSensor
    sensor: ContactSensor = env.scene[sensor_name]
    air = sensor.data.current_air_time  # (num_envs, num_feet)
    ...
    asset: Entity = env.scene[asset_cfg.name]
    if not hasattr(env, "_pitch_swing_ids"):
        ids, _ = asset.find_joints([r"^(left|right)_hip_pitch$"])
        env._pitch_swing_ids = torch.tensor(ids, device=env.device)
    ids = env._pitch_swing_ids
    dev = torch.abs(
        asset.data.joint_pos[:, ids] - asset.data.default_joint_pos[:, ids]
    )
    excess = torch.clamp(dev - free_band, min=0.0)
    per_env = torch.sum(swing * excess, dim=1)
...
class SmoothCrouchCommand(UniformPoseCommand):
    """Explicit stand/crouch buckets, with a physical height slew per second."""
    def __init__(self, cfg, env):
        super().__init__(cfg, env)
        self._height_target = torch.zeros(self.num_envs, device=self.device)

    def _resample_command(self, env_ids):
        if len(env_ids) == 0:
            return
        values = torch.tensor(self.cfg.height_buckets, device=self.device)
        choices = torch.randint(len(values), (len(env_ids),), device=self.device)
        self._height_target[env_ids] = values[choices]
        # Episode reset cannot inherit an old crouch reference. Ordinary
        # command resampling preserves continuity from the current reference.
        fresh = self._env.episode_length_buf[env_ids] <= 1
        self._command[env_ids[fresh]] = 0.

    def _update_command(self):
        delta = self.cfg.height_slew_m_s * self._env.step_dt
        self._command[:, 2] += (self._height_target-self._command[:, 2]).clamp(-delta, delta)


@dataclass(kw_only=True)
class SmoothCrouchCommandCfg(UniformPoseCommandCfg):
    height_buckets: tuple[float, ...] = (0., -.01, -.02)
    height_slew_m_s: float = .02

    def build(self, env):
        return SmoothCrouchCommand(self, env)


class CrouchCycleCommand(SmoothCrouchCommand):
    """One sampled depth per episode; stand 2s, lower/hold to 7s, return.

    A private elapsed counter avoids runner init_at_random_ep_len jumping
    straight into a deep target. Partial reset changes selected rows only.
    """
    def __init__(self, cfg, env):
        super().__init__(cfg, env)
    ...
    def _update_command(self):
        phase = self.elapsed.remainder(12.)
        active = (phase >= 2.) & (phase < 7.)
        desired = torch.where(active, self._height_target, 0.)
        delta = self.cfg.height_slew_m_s*self._env.step_dt
        self._command[:, 2] += (desired-self._command[:, 2]).clamp(-delta, delta)
        self.elapsed += self._env.step_dt


@dataclass(kw_only=True)
class CrouchCycleCommandCfg(SmoothCrouchCommandCfg):
    height_buckets: tuple[float, ...] = (0., -.02, -.03, -.04)

    def build(self, env):
        return CrouchCycleCommand(self, env)


from mjlab.tasks.velocity.mdp.rewards import variable_posture as _VariablePosture


class crouch_reference_posture(_VariablePosture):
    """Keep the original leg tolerances; shift HOME using audited fixed-foot IK.

    The table is reward-only, not an action trajectory or privileged actor input.
    Never modify asset defaults: action offsets and reset HOME stay invariant.
    """
    def __init__(self, cfg, env):
        super().__init__(cfg, env)
        asset = env.scene[cfg.params['asset_cfg'].name]
        ids, names = asset.find_joints(cfg.params['reference_names'], preserve_order=True)
        assert list(names) == list(cfg.params['reference_names'])
        self.reference_ids = torch.tensor(ids, device=env.device)
        self.table = torch.tensor(cfg.params['reference_table'], device=env.device)
        self.home = asset.data.default_joint_pos.clone()

    def __call__(self, env, reference_table, reference_names, **kwargs):
        depth = (-env.command_manager.get_command('body_pose')[:, 2]/.01).clamp(0, 4)
        lo = depth.floor().long().clamp(max=3)
        fraction = (depth-lo)[:, None]
        target = self.table[lo]*(1-fraction)+self.table[lo+1]*fraction
        self.default_joint_pos = self.home.clone()
        self.default_joint_pos[:, self.reference_ids] = target
        return super().__call__(env, **kwargs)


def local_body_height_tracking(env, command_name="body_pose", sensor_name="body_clearance",
                               nominal_height=.117182, std=.015):
    """Height relative to CURRENT ground under the trunk, never spawn Z.

    The terrain ray is a reward-only privileged quantity, not actor input.
    Separate height reward avoids diluting crouch among five inactive axes.
    """
    height = env.scene[sensor_name].data.heights[:, 0]
    target = nominal_height+env.command_manager.get_command(command_name)[:, 2]
    valid = torch.isfinite(height)
    error = torch.nan_to_num(height-target, nan=1., posinf=1., neginf=-1.)
    return torch.exp(-(error/std)**2)*valid


def reset_calibrated_stand(env, env_ids, yaw_range=(-math.pi,math.pi)):
    """Reset physics to a sampled sensor-calibrated v7 standing snapshot.

    Global yaw rotation is arbitrary; this known reset transform is used by
    reward scoring only. IMU control uses the bank's measured local frame.
    """
    if env_ids is None: env_ids = torch.arange(env.num_envs,device=env.device)
    if len(env_ids) == 0: return
    term = env.action_manager.get_term('joint_pos')
    bank = term.bank
    selected = torch.randint(len(bank['joint_pos']),(len(env_ids),),device=env.device)
    term.bank_index[env_ids] = selected
    yaw = torch.empty(len(env_ids),device=env.device).uniform_(*yaw_range)
    zero = torch.zeros_like(yaw)
    rotate = quat_from_euler_xyz(zero,zero,yaw)
    pose = bank['root_qpos'][selected].clone()
    pose[:,:3] = quat_apply(rotate,pose[:,:3])+env.scene.env_origins[env_ids]
    original_q = pose[:,3:7].clone()
    pose[:,3:7] = quat_mul(rotate,original_q)
    velocity = bank['root_qvel'][selected].clone()
    velocity[:,:3] = quat_apply(rotate,velocity[:,:3])
    velocity[:,3:] = quat_apply(pose[:,3:7],velocity[:,3:])
    robot = env.scene['robot']
    robot.write_root_state_to_sim(torch.cat((pose,velocity),-1),env_ids=env_ids)
    robot.write_joint_state_to_sim(bank['joint_pos'][selected],bank['joint_vel'][selected],
        joint_ids=term._target_ids,env_ids=env_ids)
    term.reward_heading_zero[env_ids] = yaw+bank['heading_world_zero'][selected]


class TransitionTwistCommand(CommandTerm):
    """40% gait, 10% stand, 25% crouch-return, 25% crouch-walk-stop."""
    def __init__(self,cfg,env):
        super().__init__(cfg,env)
        self._command = torch.zeros(self.num_envs,3,device=self.device)
        self.elapsed = torch.zeros(self.num_envs,device=self.device)
        self.mode = torch.zeros(self.num_envs,dtype=torch.long,device=self.device)
        self.depth = torch.zeros(self.num_envs,device=self.device)
        self.request = torch.zeros_like(self._command)

    @property
    def command(self): return self._command

    def compute(self,dt):
        # env.reset() calls compute(0) for all rows; that is not a control step.
        if dt > 0: super().compute(dt)

    def _resample_command(self,env_ids):
        r=torch.rand(len(env_ids),device=self.device)
        self.mode[env_ids]=(r>=.4).long()+(r>=.5).long()+(r>=.75).long()
        depths=torch.tensor([-.02,-.03,-.04],device=self.device)
        self.depth[env_ids]=depths[torch.randint(3,(len(env_ids),),device=self.device)]
        self.request[env_ids,0]=torch.empty(len(env_ids),device=self.device).uniform_(-.1,.55)
        self.request[env_ids,1]=torch.empty(len(env_ids),device=self.device).uniform_(-.03,.03)
        self.request[env_ids,2]=torch.empty(len(env_ids),device=self.device).uniform_(-.35,.35)
    ...
    def _update_command(self):
        self.elapsed += self._env.step_dt
        wanted=torch.where((self.mode==0)[:,None],self.request,0.)
        resume=(self.mode==3)&(self.elapsed>=11.)&(self.elapsed<17.)
        wanted[resume]=torch.tensor([.35,0.,0.],device=self.device)
        # Shared predictable user-command ramp; height command has its own slew.
        self._command += (wanted-self._command).clamp(-.03,.03)

    def _update_metrics(self): pass


@dataclass(kw_only=True)
class TransitionTwistCommandCfg(CommandTermCfg):
    def build(self,env): return TransitionTwistCommand(self,env)


class TerrainTransitionTwist(TransitionTwistCommand):
    """Flat posture retention; three speed buckets on real up/down steps."""
    def _resample_command(self,env_ids):
        super()._resample_command(env_ids)
        terrain=self._env.scene.terrain.terrain_types[env_ids]>0
        ids=env_ids[terrain]
        self.mode[ids]=0
        speeds=torch.tensor([.2,.35,.55],device=self.device)
        self.request[ids]=0
        self.request[ids,0]=speeds[torch.randint(3,(len(ids),),device=self.device)]

    def _update_command(self):
        self.elapsed+=self._env.step_dt
        walking=(self.mode==0)&(self.elapsed<17.)
        wanted=torch.where(walking[:,None],self.request,0.)
        resume=(self.mode==3)&(self.elapsed>=11.)&(self.elapsed<17.)
        wanted[resume]=torch.tensor([.35,0.,0.],device=self.device)
        self._command+=(wanted-self._command).clamp(-.03,.03)
...
@dataclass(kw_only=True)
class TerrainTransitionTwistCfg(CommandTermCfg):
    def build(self,env):return TerrainTransitionTwist(self,env)


def lane_body_height_tracking(env):
    height=env.scene['body_clearance'].data.heights[:,0]
    desired=.117182+env.command_manager.get_command('body_pose')[:,2]
    # Step edges change ground-under-trunk discretely. Preserve the stricter
    # crouch score on flat cells; do not punish legitimate stair traversal.
    std=torch.where(env.scene.terrain.terrain_types>0,.02,.01)
    valid=torch.isfinite(height)
    error=torch.nan_to_num(height-desired,nan=1.,posinf=1.,neginf=-1.)
    return torch.exp(-(error/std)**2)*valid


def lane_boundary(env):
    position=env.scene['robot'].data.root_link_pos_w-env.scene.env_origins
    return (position[:,0]<-.6)|(position[:,0]>4.8)|(position[:,1].abs()>.8)


class TransitionHeightCommand(SmoothCrouchCommand):
    def compute(self,dt):
        if dt > 0: super().compute(dt)
    ...
    def reset(self,env_ids=None):
        self._command[slice(None) if env_ids is None else env_ids]=0.
        return super().reset(env_ids)

    def _update_command(self):
        twist=self._env.command_manager.get_term('twist')
        active=(twist.mode>=2)&(twist.elapsed>=2.)&(twist.elapsed<7.)
        wanted=torch.where(active,twist.depth,0.)
        delta=self.cfg.height_slew_m_s*self._env.step_dt
        self._command[:,2]+=(wanted-self._command[:,2]).clamp(-delta,delta)


@dataclass(kw_only=True)
class TransitionHeightCommandCfg(SmoothCrouchCommandCfg):
    def build(self,env): return TransitionHeightCommand(self,env)


def navigation_head_attitude(env,std=(.5,.3,.3)):
    """Reward-only actual IMU attitude versus the controller's navigation target.

    Site orientation is privileged scoring only. The controller never receives
    it. The reset world yaw bridge comes from the known reset transform.
    """
    from mjlab_microduck.head_attitude_torch import exp_so3,log_so3
    term=env.action_manager.get_term('joint_pos')
    sid=env.sim.mj_model.site('robot/head_imu').id
    actual=env.sim.data.site_xmat[:,sid].reshape(env.num_envs,3,3)
    reference=torch.zeros(env.num_envs,3,device=env.device)
    reference[:,2]=term.reward_heading_zero+term.yaw_reference
    error=log_so3(actual.transpose(-1,-2)@exp_so3(reference))
    scale=torch.tensor(std,device=env.device)
    return torch.exp(-torch.sum((error/scale)**2,dim=-1))


def neck_only_pose_tracking(env,std=.3):
    """Retain neck height command, release the 3 IMU-controlled head joints."""
    robot=env.scene['robot'];ids,_=robot.find_joints(['neck_pitch'])
    error=robot.data.joint_pos[:,ids[0]]-robot.data.default_joint_pos[:,ids[0]]
    error-=env.command_manager.get_command('head_pose')[:,0]
    return torch.exp(-(error/std)**2)


def body_clearance_below(env, sensor_name="body_clearance", minimum_height=.055):
    """Terrain-relative clearance termination for slopes and steps."""
    return env.scene[sensor_name].data.heights[:, 0] < minimum_height
...
def limb_pose_tracking(
    ...
    """Per-joint Gaussian reward for matching a commanded joint-angle vector.

    Generic version of head_pose_tracking for limb groups without backlash
    (MicroDinosaur arms/tail): measured angle is the servo joint itself, command is a
    vector of DELTAS from the default joint positions, per-joint
    exp(-(err/std)^2) averaged so partial tracking gives partial reward.
    """
    asset: Entity = env.scene[asset_cfg.name]
    cmd = env.command_manager.get_command(command_name)  # (N, K)
    cache = f"_limb_{command_name}_ids"
    if not hasattr(env, cache):
        ids, names = asset.find_joints_by_actuator_names(list(joint_patterns))
        setattr(env, cache, torch.tensor(ids, device=env.device, dtype=torch.long))
    ids = getattr(env, cache)
    joint_pos = asset.data.joint_pos
    actual = joint_pos[:, ids] - asset.data.default_joint_pos[:, ids]
    err = actual - cmd
...
def stand_still_penalty(
    ...
    """Motion cost that only applies when the twist command is ~zero.

    Fixes the observed MicroDinosaur failure (2026-09-12, user: "原地站立的时候会有点抖",
    "在地上有静态的旋转"): the policy drifted/yawed about -6 deg/s at zero command
    because nothing priced residual motion while standing.

    Rewritten 2026-09-12 (same day): the first version also priced the pitch/roll
    rate and all 19 joint velocities.  A standing biped must actively make those
    corrections, so the term fought the controller and the *shared* policy
    degraded globally -- reward 129 -> 62 over ~300 iterations, action-rate
    penalty -1.9 -> -3.5, mean_action_acc 0.40 -> 0.58 (jitter), arm/head/tail
    gesture tracking down ~25%.  Single-variable probes pinned it down: with
    `MICRODINO_PROBE=nostill` the same warm start holds 155-157 (identical to
    the pre-change recipe) while the other two changes (mirror loss, action-scale
    DR) changed nothing.  Only the two qualities the user actually reported are
    priced now: in-place rotation (yaw rate) and horizontal drift.
    """
    cmd = env.command_manager.get_command(command_name)
    moving = cmd.abs().sum(dim=-1) > command_threshold
    asset: Entity = env.scene[asset_cfg.name]
    lin = torch.linalg.norm(asset.data.root_link_lin_vel_b[:, :2], dim=-1)
    yaw = asset.data.root_link_ang_vel_b[:, 2].abs()
    cost = lin_gain * lin + yaw_gain * yaw
    return torch.where(moving, torch.zeros_like(cost), cost)


class lean_drift_penalty(ManagerTermBase):
    """Persistent lateral lean cost with per-environment episode state.

    Keep the existing EMA, command gate and reward scale. RewardManager calls
    reset only for the environments starting a new episode; other histories
    must continue unchanged. This state belongs to this term, not a module cache.
    """
    ...
    def reset(self, env_ids=None):
        self.ema[slice(None) if env_ids is None else env_ids] = 0.0

    def __call__(self, env, ema_alpha=0.01, command_name="twist",
                 asset_cfg=_DEFAULT_ASSET_CFG):
        moving = env.command_manager.get_command(command_name).abs().sum(dim=-1) > 0.05
        asset = env.scene[asset_cfg.name]
        q = asset.data.root_link_quat_w
        g_w = torch.tensor([0.0, 0.0, -1.0], device=q.device).expand(q.shape[0], 3)
        lean = -quat_apply_inverse(q, g_w)[:, 1]
        self.ema.mul_(1.0 - ema_alpha).add_(lean, alpha=ema_alpha)
        return torch.where(moving, self.ema.abs(), torch.zeros_like(self.ema))


class contact_timing_asymmetry(ManagerTermBase):
    """Left/right contact-duty cost with resettable per-environment EMA."""

    def __init__(self, cfg, env):
        super().__init__(env)
        found = env.scene[cfg.params["sensor_name"]].data.found
        self.ema = torch.full_like(found, 0.5, dtype=torch.float)

    def reset(self, env_ids=None):
        self.ema[slice(None) if env_ids is None else env_ids] = 0.5

    def __call__(self, env, sensor_name, ema_alpha=0.005,
                 command_name="twist", asset_cfg=_DEFAULT_ASSET_CFG):
        del asset_cfg
        found = (env.scene[sensor_name].data.found > 0).float()
        if found.shape[1] < 2:
            return torch.zeros(found.shape[0], device=found.device)
        moving = env.command_manager.get_command(command_name).abs().sum(dim=-1) > 0.05
        self.ema.mul_(1.0 - ema_alpha).add_(found, alpha=ema_alpha)
        cost = (self.ema[:, 0] - self.ema[:, 1]).abs()
        return torch.where(moving, cost, torch.zeros_like(cost))


def _nominal_head_home(env, asset_cfg):
    """Heading-free head HOME quaternion from default joint FK, without stepping.

    Use a separate CPU MjData on the nominal compiled model. Never capture the
    live first rollout frame: it can contain exploratory actions, randomized
    dynamics and an already moving head. Never mutate the training state.
    """
    asset = env.scene[asset_cfg.name]
    model = env.sim.mj_model
    nominal = mujoco.MjData(model)
    indexing = asset.indexing
    if not asset.is_fixed_base:
        addresses = indexing.free_joint_q_adr.cpu().numpy()
        nominal.qpos[addresses] = asset.data.default_root_state[0, :7].cpu().numpy()
    nominal.qpos[indexing.joint_q_adr.cpu().numpy()] = asset.data.default_joint_pos[0].cpu().numpy()
    mujoco.mj_kinematics(model, nominal)
    bodies = indexing.body_ids[asset_cfg.body_ids].reshape(-1)
    if bodies.numel() != 1:
        raise ValueError("head_world_gaze_tracking requires exactly one head body")
    q_head = torch.tensor(nominal.xquat[int(bodies[0])].copy(), device=env.device, dtype=torch.float)[None]
    q_root = torch.tensor(nominal.xquat[indexing.root_body_id].copy(), device=env.device, dtype=torch.float)[None]
    yaw = _yaw_of(q_root)
    zeros = torch.zeros_like(yaw)
    return quat_mul(quat_from_euler_xyz(zeros, zeros, -yaw), q_head)


class head_world_gaze_tracking(ManagerTermBase):
    """Track deterministic nominal head attitude relative to current body heading.

    Preserve quaternion composition and command channel conventions. The head
    frame is near an Euler singularity; do not decompose its HOME into Euler
    angles. The fixed reference is computed before training rollouts and remains
    the same across resets, exploratory first actions and random spawn headings.
    """

    def __init__(self, cfg, env):
        super().__init__(env)
        asset_cfg = cfg.params.get("asset_cfg", _DEFAULT_ASSET_CFG)
        self.home_quat = _nominal_head_home(env, asset_cfg)

    def __call__(self, env, command_name="head_pose", std=0.5,
                 pitch_channel=1, yaw_channel=2, roll_channel=3,
                 asset_cfg=_DEFAULT_ASSET_CFG):
        asset = env.scene[asset_cfg.name]
        cmd = env.command_manager.get_command(command_name)
        q_head = asset.data.body_link_quat_w[:, asset_cfg.body_ids, :].squeeze(1)
        yaw_trunk = _yaw_of(asset.data.root_link_quat_w)
        zeros = torch.zeros_like(yaw_trunk)
        q_off = quat_from_euler_xyz(cmd[:, roll_channel], cmd[:, pitch_channel], cmd[:, yaw_channel])
        q_des = quat_mul(quat_from_euler_xyz(zeros, zeros, yaw_trunk),
                         quat_mul(self.home_quat.expand_as(q_head), q_off))
        err = quat_error_magnitude(q_head, q_des)
        return torch.exp(-torch.square(err) / (std * std))



def _yaw_of(quat: torch.Tensor) -> torch.Tensor:
    """World yaw angle of a (w, x, y, z) quaternion batch."""
    w, x, y, z = quat[:, 0], quat[:, 1], quat[:, 2], quat[:, 3]
    return torch.atan2(2.0 * (w * z + x * y), 1.0 - 2.0 * (y * y + z * z))


def head_gaze_penalty(
    env: ManagerBasedRlEnv,
    asset_cfg: SceneEntityCfg = _DEFAULT_ASSET_CFG,
) -> torch.Tensor:
    """Head stillness in the WORLD frame (gaze stability), always active.

    2026-09-13 user request: "走路的时候头尽量稳定……保持视角稳定". The
    head_camera site is trunk-mounted, so a stable view needs a quiet trunk
    (body_ang_vel) AND a head that does not slew on top of it.  This term
    prices the head bodies' world-frame angular velocity on ALL axes (a
    rotating view is exactly the complaint) plus horizontal head bob.
    Deliberately NOT gated by the command: the head must be steady while
    walking, not only while standing.  Normalized per body so adding bodies
    does not change the term's scale.
    """
    asset: Entity = env.scene[asset_cfg.name]
    ang = asset.data.body_link_ang_vel_w[:, asset_cfg.body_ids, :]
    n = max(1, len(asset_cfg.body_ids))
    # v2 (after the 120-iter probe): angulation only, MEAN over the bodies.
    # The first version also priced horizontal head bob and SUMMED over the
    # three head bodies (raw cost ~8 -> the term fought the head command
    # channel AND the gait bob at weight 0.5 and the warm start collapsed to
    # reward -300).  A rotating VIEW is the complaint; small translation is
    # the gait, not the complaint.
    return torch.square(ang).sum(dim=-1).mean(dim=-1)


def stand_jitter_penalty(
    env: ManagerBasedRlEnv,
    command_name: str = "twist",
    command_threshold: float = 0.05,
    asset_cfg: SceneEntityCfg = _DEFAULT_ASSET_CFG,
) -> torch.Tensor:
    """Zero-command joint-speed cost (the standing fidget).

    Targets the measured standing fidget (action jerk 0.06 per step at 50 Hz,
    feet oscillating without ever lifting above 3 mm) without touching the
    gait: gated by |command| like stand_still_penalty, and prices ONLY joint
    speed.  The 2026-09-12 collapse came from a version that also priced the
    body pitch/roll rate and all joint speeds at total weight 1.5 -- fighting
    the balance controller.  Keep this weight small (~0.4): a nudge against
    fidgeting, not a brace.
    """
    cmd = env.command_manager.get_command(command_name)
    moving = cmd.abs().sum(dim=-1) > command_threshold
    asset: Entity = env.scene[asset_cfg.name]
    # v2 (after the 120-iter probe): MEAN over the 19 joints, not a sum -- the
    # sum put the raw cost at ~28 (RMS joint speed ~1.2 rad/s while standing,
    # i.e. real fidget) and at weight 0.4 that alone destabilized the warm
    # start.  Mean keeps the same signal at ~1/19 the scale.
    cost = torch.square(asset.data.joint_vel).mean(dim=-1)
    return torch.where(moving, torch.zeros_like(cost), cost)


def stand_slip_penalty(
    env: ManagerBasedRlEnv,
    sensor_name: str = "feet_ground_contact",
    command_name: str = "twist",
    command_threshold: float = 0.05,
    ...
    """Foot-slip cost that applies ONLY when the twist command is ~zero.

    mjlab's `feet_slip` gates the opposite way -- it prices slip only while
    MOVING (`active = total_command > threshold`) -- so at zero command the
    stance feet grind for free.  Measured 2026-09-12 with
    scripts/microdinosaur_measure.py on the fine model, kp=7/kv=0.4:

        warm start model_13500 : drift  +0.4 deg / 5 s, foot lift 1.3 / 0.9 mm
        after 800 iters        : drift +71.2 deg / 5 s, foot lift 0.7 / 0.9 mm

    The feet are NOT lifting (not marching): the policy rotates at +14 deg/s
    with them planted, i.e. the pivot-by-grinding turn strategy the user
    accepted for low-speed turning runs away at standstill -- exactly the
    reported "static rotation on the ground".  Same functional form as
    mdp.feet_slip with the gate inverted, so walking and turning envs (non-zero
    command) are untouched and the shared gait is not distorted the way the
    body-motion stillness penalty distorted it.
    """
    asset: Entity = env.scene[asset_cfg.name]
    contact_sensor = env.scene[sensor_name]
    cmd = env.command_manager.get_command(command_name)
    total = torch.linalg.norm(cmd[:, :2], dim=1) + cmd[:, 2].abs()
    idle = (total <= command_threshold).float()
    in_contact = (contact_sensor.data.found > 0).float()
    foot_vel_xy = asset.data.site_lin_vel_w[:, asset_cfg.site_ids, :2]
    vel_sq = torch.square(torch.linalg.norm(foot_vel_xy, dim=-1))
    return torch.sum(vel_sq * in_contact, dim=1) * idle
...
def randomize_action_scale(
    ...
    """Per-env action-amplitude randomization (gait-size robustness).

    Training at scale 1.0 only produced a gait that breaks above ~1.2 and turns
    asymmetric at 1.6 (right foot lifted 5.6 mm vs left 3.5 mm).  Randomizing the
    action scale per episode teaches one policy that keeps left-right
    coordination across stride sizes instead of a single tuned amplitude.
    """
    if env_ids is None:
        env_ids = torch.arange(env.num_envs, device=env.device, dtype=torch.long)
    term = None
    for attr in ("terms", "_terms"):
        bag = getattr(env.action_manager, attr, None)
        if isinstance(bag, dict) and action_name in bag:
            term = bag[action_name]
            break
    if term is None:
        return
    if not isinstance(term._scale, torch.Tensor):
        term._scale = torch.ones(env.num_envs, term.action_dim, device=env.device)
    lo, hi = scale_range
    sample = torch.rand(len(env_ids), device=env.device) * (hi - lo) + lo
    term._scale[env_ids] = sample.unsqueeze(-1)
...
class IndependentSquatCommand(CommandTerm):
    """Zero-speed squat/hold/return requests with smooth ramps and idle trials."""
    def __init__(self, cfg, env):
        super().__init__(cfg, env)
        self.elapsed = torch.zeros(self.num_envs, device=self.device)
        self.depth = torch.zeros_like(self.elapsed)
        self.return_at = torch.full_like(self.elapsed, 7.)
    ...
    def _resample_command(self, env_ids):
        n = len(env_ids)
        self.depth[env_ids] = torch.empty(n, device=self.device).uniform_(.015, .025)
        idle = torch.rand(n, device=self.device) < .15
        self.depth[env_ids[idle]] = 0
        self.return_at[env_ids] = torch.where(torch.rand(n, device=self.device) < .2, 4., 7.)

    def _update_command(self):
        self.elapsed += self._env.step_dt
        if self.cfg.width != 6: return
        down = ((self.elapsed-1.)/3.).clamp(0, 1)
        up = ((self.elapsed-self.return_at)/3.).clamp(0, 1)
        smooth = lambda t: t*t*(3-2*t)
        self._command[:, 2] = -self.depth*smooth(down)*(1-smooth(up))

    def _update_metrics(self): pass


@_dataclass(kw_only=True)
class IndependentSquatCommandCfg(CommandTermCfg):
    width: int = 6
    def build(self, env): return IndependentSquatCommand(self, env)


class independent_squat_tracking(ManagerTermBase):
    """Joint and height agreement must coincide with upright bilateral support.

    The reference is reward-only. The actor still predicts 19 position actions;
    there is no injected leg trajectory, teleportation or privileged actor input.
    """
    def __init__(self, cfg, env):
        super().__init__(env)
        self.names = cfg.params['reference_names']
        self.ids, _ = env.scene['robot'].find_joints(self.names, preserve_order=True)
        self.legs = [i for i, n in enumerate(self.names) if n.startswith(('left_', 'right_'))]
        self.table = torch.tensor(cfg.params['reference_table'], device=env.device)

    def __call__(self, env, reference_names, reference_table, step_m=.005):
        action = env.action_manager.get_term('joint_pos')
        depth = (-env.command_manager.get_command('body_pose')[:, 2]/step_m).clamp(0, len(self.table)-1)
        lo = depth.floor().long().clamp(max=len(self.table)-2)
        fraction = (depth-lo)[:, None]
        target = self.table[lo]*(1-fraction)+self.table[lo+1]*fraction
        target += action.bank['joint_pos'][action.bank_index]-self.table[0]
        robot = env.scene['robot']
        measured = (robot.data.joint_pos+robot.data.encoder_bias)[:, self.ids]
        pose_error = ((measured[:, self.legs]-target[:, self.legs])/.18).square().mean(-1)
        nominal = action.bank['root_qpos'][action.bank_index, 2]
        z_target = nominal+env.command_manager.get_command('body_pose')[:, 2]
        height_error = ((env.scene['body_clearance'].data.heights[:, 0]-z_target)/.008).square()
        support = env.scene['feet_ground_contact'].data.found.reshape(env.num_envs, -1).gt(0).all(-1)
        upright = robot.data.projected_gravity_b[:, 2] < -math.cos(math.radians(15))
        nonfoot = env.scene['nonfoot_ground'].data.found.reshape(env.num_envs, -1).gt(0).any(-1)
        return torch.exp(-pose_error-height_error)*support*upright*(~nonfoot)


def independent_squat_envelope(env):
    """Task operating bound, not a change to physical joint stops."""
    action = env.action_manager.get_term('joint_pos')
    q = env.scene['robot'].data.joint_pos[:, action._target_ids]
    ankle = [action._target_names.index(n) for n in ('left_ankle', 'right_ankle')]
    return q[:, ankle].abs().amax(-1) > math.radians(55)


@requires_model_fields('actuator_forcerange')
def reset_squat_motor_envelope(env, env_ids):
...
class motion_clear_running(ManagerTermBase):
    """Reward both entire soles clear, with no body-floor contacts.

    This control-tick shaping term is not the acceptance metric: independent
    evaluation checks continuous flight at every physics substep.
    """
    def __init__(self,cfg,env):
        super().__init__(env)
        import numpy as np
        ...
        model=env.sim.mj_model
        self.gids=[model.geom('robot/'+side+'_foot_collision').id for side in ('left','right')]
        self.vertices=[]
        for gid in self.gids:
            mesh=model.geom_dataid[gid];start=model.mesh_vertadr[mesh]
            v=model.mesh_vert[start:start+model.mesh_vertnum[mesh]].copy()
            self.vertices.append(torch.tensor(v[ConvexHull(v).vertices],device=env.device))

    def clearance(self,env):
        gaps=[]
        for gid,v in zip(self.gids,self.vertices):
            rotation=env.sim.data.geom_xmat[:,gid].reshape(-1,3,3)
            gaps.append((rotation[:,2,:]@v.T).amin(-1)+env.sim.data.geom_xpos[:,gid,2])
        return torch.stack(gaps,-1).amin(-1)

    def __call__(self,env):
        clear=self.clearance(env)>.002
        support=env.scene['robot_ground'].data.found.reshape(env.num_envs,-1).gt(0).any(-1)
        cmd=env.command_manager.get_command('twist')[:,0]
        robot=env.scene['robot']
        return clear*(~support)*(cmd>.4)*(robot.data.projected_gravity_b[:,2]<-.94)*torch.exp(-((robot.data.root_link_lin_vel_b[:,0]-cmd)/.25)**2)


class motion_running_sole_lift(motion_clear_running):
    """Dense sole-clearance shaping only during physically unsupported motion.

    No COM/root-height proxy. The first 0.2 mm earns nothing, then the reward
    increases up to 5 mm. Formal acceptance remains >2 mm for 40 ms.
    """
    def __call__(self,env):
        gap=self.clearance(env)
        level=((gap-.0002)/.0048).clamp(0,1)
        force=env.scene['whole_ground_force'].data.force.reshape(env.num_envs,-1,3)
        support=force[...,2].abs().sum(-1)
        robot=env.scene['robot'];cmd=env.command_manager.get_command('twist')[:,0]
        speed=robot.data.root_link_lin_vel_b[:,0]
        return level*(support<.05)*(cmd>.4)*(speed>.2)*(robot.data.projected_gravity_b[:,2]<-.94)*torch.exp(-((speed-cmd)/.25)**2)


def motion_stable_stand(env,stop_only=False):
    robot=env.scene['robot']
    gravity=robot.data.projected_gravity_b
    support=env.scene['feet_ground_contact'].data.found.reshape(env.num_envs,-1).gt(0).all(-1)
    nonfoot=env.scene['nonfoot_ground'].data.found.reshape(env.num_envs,-1).gt(0).any(-1)
    z=robot.data.root_link_pos_w[:,2]-env.scene.env_origins[:,2]
    value=torch.exp(-((z-.1135)/.012)**2-gravity[:,:2].square().sum(-1)/.04
        -robot.data.root_link_lin_vel_b.square().sum(-1)/.0025
        -robot.data.root_link_ang_vel_b.square().sum(-1)/.25)
    value*=support*(~nonfoot)*(gravity[:,2]<-.94)
    if stop_only:value*=env.command_manager.get_command('twist').norm(dim=-1)<.03
    return value


def motion_recovery_posture(env):
    """Bounded dense shaping. Full success still requires stable foot support."""
    robot=env.scene['robot']
    upright=(-robot.data.projected_gravity_b[:,2]).clamp(0,1)
    z=robot.data.root_link_pos_w[:,2]-env.scene.env_origins[:,2]
    height=((z-.025)/(.1135-.025)).clamp(0,1)
    return upright.square()*(.25+.75*height)


def motion_recovery_posture_dense(env):
    """Orientation feedback also exists beyond 90 degrees of body tilt.

    The earlier upright-clamped recipe had a dead zone at the actual 99-103
    degree fallen starts. The separately gated standing reward is unchanged.
    """
    robot=env.scene['robot']
    upright=((1-robot.data.projected_gravity_b[:,2])/2).clamp(0,1)
    z=robot.data.root_link_pos_w[:,2]-env.scene.env_origins[:,2]
    height=((z-.025)/(.1135-.025)).clamp(0,1)
    return upright.square()*(.25+.75*height)


def recovery_head_envelope(env):
    action=env.action_manager.get_term('joint_pos')
    q=env.scene['robot'].data.joint_pos[:,action._target_ids[action._target_names.index('head_pitch')]]
    return (q<math.radians(-2)) | (q>math.radians(29))


NECK_BATTERY_CLEARANCE_DEG = 35.
"""Target-side clamp on neck_pitch during get-up. CAD breach measured at
40.75 deg (nominal head yaw/roll) and 38.50 deg at the head_roll extremes, so the
clamp is set a full 3.5 deg below the worst case. Sourced from
`recovery_bounds` so the CPU evaluator clamps the identical value."""
...
def recovery_neck_battery_envelope(env):
    """CAD head/battery clearance as a joint envelope.

    `S288_head_roll_CASE` (body jaw_soft) enters `JMP_3S4000_90x42x27_ENVELOPE`
    (body trunk_base) once neck_pitch passes ~40.8 deg, tightening to ~38.5 deg at
    the head_roll extremes. Those two bodies are ancestor and descendant, so MuJoCo
    filters their contacts by construction and no collision proxy can represent the
    constraint; a joint envelope is the only enforceable form.
    """
    action=env.action_manager.get_term('joint_pos')
    q=env.scene['robot'].data.joint_pos[:,action._target_ids[action._target_names.index('neck_pitch')]]
    return q>math.radians(NECK_BATTERY_TERMINATE_DEG)


def recovery_potential(env):
    """Scalar get-up progress in [0,1]: upright orientation blended with height."""
    robot=env.scene['robot']
    upright=((1-robot.data.projected_gravity_b[:,2])/2).clamp(0,1)
    z=robot.data.root_link_pos_w[:,2]-env.scene.env_origins[:,2]
    height=((z-.025)/(.1135-.025)).clamp(0,1)
...
class motion_recovery_progress:
    """Potential-based dense get-up shaping that cannot be farmed by lying still.

    The previous recipe returned a bounded *absolute* posture value. From a 100 deg
    fallen start that value is already ~0.14 of its 1.0 maximum, so the gradient
    across the ~100 deg of rotation a get-up needs is tiny, while a collapsed body
    that stops moving keeps collecting it forever. Measured consequence: the
    released candidate settles into a static pose within 0.5 s and holds it for the
    remaining 11.5 s (root speed ~3e-7 m/s, feet never loaded).

    This term pays for the *change* in potential instead, so every motion toward
    upright is rewarded, a static collapsed pose earns exactly zero, and the
    `stuck` timer it exposes lets the task end an episode that makes no progress.
    Returns d(potential)/dt because the reward manager already multiplies by dt.
    """

    def __init__(self,cfg,env):
        self.dt=env.step_dt
        self.tolerance=float(cfg.params.get('tolerance',.02))
        self.stuck_seconds=float(cfg.params.get('stuck_seconds',3.))
        self.upright_at=float(cfg.params.get('upright_at',.9))
        self.prev=None
        self.best=torch.zeros(env.num_envs,device=env.device)
        self.stuck=torch.zeros(env.num_envs,device=env.device)
        env.getup_stuck=self.stuck
        env.getup_stuck_seconds=self.stuck_seconds
    ...
    def __call__(self,env,**params):
        # `params` is already consumed in __init__ via cfg.params; the manager
        # forwards them again on every call.
        phi=recovery_potential(env)
        if self.prev is None or self.prev.shape!=phi.shape:
            self.prev=phi.detach().clone();self.best=phi.detach().clone()
            return torch.zeros_like(phi)
        current=phi.detach()
        # The first step after a reset belongs to a new episode; paying the jump
        # from the previous episode's final potential would be a spurious bonus.
        fresh=env.episode_length_buf<=1
        self.prev[fresh]=current[fresh]
        self.best[fresh]=current[fresh]
        self.stuck[fresh]=0.
        improved=current>self.best+self.tolerance
        self.best=torch.where(improved,current,self.best)
        # Standing still is not "stuck"; only a body that is still down and not
        # getting closer counts, so an episode that starts near upright is free to
        # collect the standing reward for its full length. Updated in place: the
        # termination reads this exact tensor through env.getup_stuck.
        self.stuck.copy_(torch.where(improved | (current>=self.upright_at),
            torch.zeros_like(self.stuck),self.stuck+self.dt))
        delta=(phi-self.prev)/self.dt
        self.prev=current.clone()
        return torch.where(fresh,torch.zeros_like(delta),delta)


def recovery_no_progress(env):
    """End an episode whose potential has not improved for `stuck_seconds`."""
    stuck=getattr(env,'getup_stuck',None)
    if stuck is None:return torch.zeros(env.num_envs,dtype=torch.bool,device=env.device)
    return stuck>getattr(env,'getup_stuck_seconds',3.)


class motion_recovery_body_ground_tax:
    """Charge for resting the chassis on the floor, ramped in over training.

    The potential alone does not forbid a *propped* get-up: at iteration 17000 three
    of four fall directions converged to the same pose (trunk at 103 mm, tilt 73 deg,
    body-ground contact present for 100% of the final two seconds) and parked there.
    That pose banks 0.72 of the potential plus most of the height term while never
    loading a foot, so it is a stable local optimum.

    A body-ground *termination* would be wrong here: the robot genuinely starts lying
    on the floor, so the constraint is a cost to escape, not a precondition. Ramping
    the weight keeps it from swamping the early exploration that teaches the get-up
    motion in the first place -- the same timed-tax pattern already used by the
    validated VelStand recipe in this project.
    """

    def __init__(self,cfg,env):
        self.delay=int(cfg.params.get('delay_steps',0))
        self.ramp=max(1,int(cfg.params.get('ramp_steps',1)))
        g=cfg.params.get('upright_gate',None)
        self.upright_gate=None if g is None else float(g)
        env.getup_ground_tax_ramp=torch.zeros(env.num_envs,device=env.device)
        self._ramp=env.getup_ground_tax_ramp

    def reset(self,env_ids=None):
        pass

    def __call__(self,env,**params):
        found=env.scene['nonfoot_ground'].data.found.reshape(env.num_envs,-1).gt(0).any(-1)
        fraction=float((int(env.common_step_counter)-self.delay)/self.ramp)
        self._ramp.fill_(min(1.,max(0.,fraction)))
        cost=self._ramp*found.float()
        # 门控到"已经站起来之后":验收只在末 2s 要求非足支地<0.2N,而过渡期用非足部位
        # 支撑是允许的(用户:"尾巴可以用来接力")。实测坐撑的唯一可行机制就是尾巴当后支点。
        if getattr(self,'upright_gate',None) is not None:
            cost=cost*(recovery_potential(env)>self.upright_gate).float()
        return cost


class recovery_ground_relapse_unused_marker:
    pass


def motion_recovery_clear_body(env, sensor_name='getup_body_force', scale_n=2.):
    """Dense reward for keeping non-foot bodies off the floor, graded by contact force.

    Measured at iteration 20500: all four fall directions stand upright, still, feet
    loaded 99%, clearing every acceptance sub-gate except body support < 0.2 N at
    0.84-0.91. The offender is the tail: `tail_pitch` sits at 0.0 mm off the floor
    while the legs are 8-48 mm clear, touching at 4-6 N for 1-2 ms at a time. A
    per-step penalty charges 4 * 0.00125 ~= 0.005 for such a tap, and a latched
    termination instead makes `motion_stable_stand` unreachable (the arm condition and
    the contact coincide, so the episode ends before any standing reward is paid).

    Superseded by `motion_recovery_body_force_cost`: sampled at the operating point
    (5 N, scale 2 N) this shape's sensitivity is only ~0.005 reward per newton per
    step, which is why a run trained on it held at 0.75-0.84 instead of improving.
    Kept for provenance of that comparison.
    """
    force=env.scene[sensor_name].data.force.reshape(env.num_envs,-1,3)
    normal=force[...,2].abs().sum(-1)
    return torch.exp(-(normal/scale_n)**2)


def motion_recovery_body_force_cost(env, sensor_name='getup_body_force', deadband_n=.2,
                                    upright_gate=None):
    """Charge non-foot floor contact force above the acceptance deadband, in newtons.

    The acceptance gate is literally "body support < 0.2 N" for the final 2 s, so the
    cost is that same criterion: zero below the deadband, then linear in the excess.

    `upright_gate` 把代价**门控到已经站起来之后**(phi>gate 才计)。为什么必须门控:
    验收只在**末 2 秒**要求非足支地 <0.2N,而过渡期用非足部位支撑是被明确允许的
    (用户:"尾巴可以用来接力")。实测坐撑起始态的唯一可行机制就是"尾巴当后支点 +
    双脚当前支点"把躯干顶立起来(尾最低 36->0mm、躯干最低 0->40mm、倾角 44.7->30.5°)。
    无条件罚它等于把这条路封死 —— 这正是坐姿那一路升不起来的原因之一。
    """
    force=env.scene[sensor_name].data.force.reshape(env.num_envs,-1,3)
    normal=force[...,2].abs().sum(-1)
    cost=-(normal-deadband_n).clamp(min=0.)
    if upright_gate is not None:
        cost=cost*(recovery_potential(env)>float(upright_gate)).float()
    return cost


def motion_recovery_tail_lift(env, upright_gate=.6, target_deg=0., floor_deg=-50.):
    """Pay for holding the tail up once the body is up.

    Measured on the standing pose: `tail_pitch` sits at -47.3 deg, hard against its
    -51.6 deg limit, i.e. the policy is *pushing* the tail down and using it as a
    third contact point. Forward kinematics over that pose: tail_pitch -20 deg leaves
    75.5 mm of tail clearance, -10 deg 109 mm, 0 deg 144 mm. So the sub-gate that
    never cleared in three rounds of force penalties (body support < 0.2 N holding at
    0.84-0.91) has a single, directly observable hinge: the tail joint.

    Linear rather than exp(): a Gaussian with a 15 deg width has a value of 5e-5 and
    a gradient of ~0 at the measured -47 deg operating point, so it pays nothing
    exactly where the policy is stuck. A ramp from `floor_deg` to `target_deg` keeps a
    constant gradient (and so a constant advantage signal) across the whole range the
    tail actually moves through.

    Gated on the potential so the get-up phase, where a tail-down push-off may be
    legitimate, is not charged. A bonus rather than a cost on purpose: it cannot make
    the standing state itself unattractive, so unlike an aggressive force penalty it
    cannot teach the policy to avoid getting up.
    """
    action=env.action_manager.get_term('joint_pos')
    q=env.scene['robot'].data.joint_pos[:,action._target_ids[action._target_names.index('tail_pitch')]]
    gate=(recovery_potential(env)>upright_gate).float()
    span=max(1e-6,target_deg-floor_deg)
    return gate*((q-math.radians(floor_deg))/math.radians(span)).clamp(0,1)


def recovery_pose_score(env, home, home_ids, std=.25, soft=.15):
    """与 v7 HOME 站姿的差值,soft-min 聚合(分数由最差关节决定)。

    用 mean 时实测:head_pitch/head_roll/jaw/arm_l/arm_r 五个关节已 0.81-0.98 白送分,
    把 left_hip_pitch/knee/ankle/neck_pitch/head_yaw/right_hip_pitch/tail_pitch 七个 0.000
    的关节托到 0.291,策略"继续磨那 15 个已经好的"就够本,永远不碰这七个 —— 聚合
    指标藏住绑定约束,与逐条 margin 的教训同源。soft-min 让最差关节说了算。
    """
    robot=env.scene['robot']
    q=(robot.data.joint_pos+robot.data.encoder_bias)[:,home_ids]
    err=(q-home[None,:])/std
    per=torch.exp(-(err*err))
    n=per.shape[-1]
    return -soft*torch.logsumexp(-per/soft,dim=-1)+soft*math.log(n)


class motion_recovery_gated_stand:
    """motion_stable_stand,但"站稳"这笔钱只在站姿接近 HOME 时才付。

    这是把账锁对的一次。前面三次改动都没推动那七个折叠关节,原因不是激励太弱,
    而是**喂错了项**:我给小的那一项(教练 hold,最高≈2.3/s)上了锁,而**大的那一项
    recovery_stand(实测 3.8/s)一直在为蜷缩站姿付钱**。策略的账本是
    "躺平 0 / 蜷缩站 3.8/s / 直腿站 8+/s",停在蜷缩站 —— 动一下先亏,不动最划算。
    把同一把锁加在 recovery_stand 上,蜷缩站立的稳定收入才真正归零。
    """

    def __init__(self,cfg,env):
        self.home=torch.tensor(cfg.params['home_pose'],device=env.device,dtype=torch.float32)
        self.gate=float(cfg.params.get('pose_gate',.25))
        self.std=float(cfg.params.get('pose_std_rad',.25))
        self.soft=float(cfg.params.get('pose_soft',.15))
        action=env.action_manager.get_term('joint_pos')
        self.ids=action._target_ids

    def reset(self,env_ids=None):
        pass

    def __call__(self,env,**params):
        pose=recovery_pose_score(env,self.home,self.ids,self.std,self.soft)
        return motion_stable_stand(env)*(pose>self.gate).float()


class motion_recovery_yaw_hold:
    """惩罚躯干偏航相对本回合初始朝向的漂移。

    为什么这是高杠杆的单点修复(实测):头 IMU 控制器锁的是"头部的**世界**朝向",
    所以躯干偏航多少、它就把 head_yaw 反向拧多少 —— 四例实测 躯干偏航 + head_yaw
    偏差 ≈ 0(+54.9/−51.2、+147.0/−146.4、−62.1/+61.9、−17.6/+13.7)。
    于是同一个上游漂移同时造成三件事:
      (1) 验收的"朝向保持"失败(实测漂移 20.8~158.8°,限 30°);
      (2) 验收的"站姿"失败(实测最大偏差达 146°,那 146° 就是 head_yaw);
      (3) 用户看图说的"站着头朝后"。
    把躯干偏航摁住,三条一起解决。
    """

    def __init__(self,cfg,env):
        self.deadband=math.radians(float(cfg.params.get('deadband_deg',10.)))
        self.yaw0=None
    ...
    def __call__(self,env,**params):
        ...
        if self.yaw0 is None or self.yaw0.shape!=yaw.shape:
            self.yaw0=yaw.detach().clone()
        fresh=env.episode_length_buf<=1
        self.yaw0[fresh]=yaw.detach()[fresh]
        d=(yaw-self.yaw0+math.pi)%(2*math.pi)-math.pi
        # 返回**正值**,由 cfg 的负权重去罚。上一版这里也带了负号,和 weight=-8 相乘
        # 变成"偏航有奖"(冒烟里该项报 +0.08→+0.43 才看出来)。
        return (d.abs()-self.deadband).clamp(min=0.)


def motion_recovery_quiet_stand(env, upright_gate=.6, omega_scale=.3, speed_scale=.05):
    """Pay for standing still, with only the gate that actually matters for it.

    Measured twice: at iter23100 the binding criterion across all eight cases was
    angular velocity (4-19%), and at iter21100 in the tail-lift round two of eight
    cases had already flipped to 平速/角速 at 91% against the 95% window requirement.
    So "stands up but will not hold still" is the second bottleneck, waiting behind
    body clearance.

    `motion_stable_stand` does contain exp(-|v|^2/.0025 - |w|^2/.25), but it is
    multiplied by the full conjunction (root height, tilt, both feet loaded, no body
    contact), so at 91% of the window it evaluates to essentially zero and supplies no
    gradient exactly where the policy needs it. This term keeps only the one gate that
    is about *this* behaviour -- the robot has to be up -- and then pays smoothly for
    damping. Gated rather than global so the get-up motion itself, which is inherently
    fast, is not charged.
    """
    robot=env.scene['robot']
    gate=(recovery_potential(env)>upright_gate).float()
    omega=robot.data.root_link_ang_vel_b.square().sum(-1)
    speed=robot.data.root_link_lin_vel_b.square().sum(-1)
    return gate*torch.exp(-omega/omega_scale**2)*torch.exp(-speed/speed_scale**2)


class motion_recovery_coach:
    """示范奖励(教练):靠拢正确站姿给小分,正确站姿上站稳给大分。

    用户看视频指出"根本没有回到正确的站立姿态",量化后成立:四个方位站起来的
    都是一个深度蜷缩姿态,与 v7 HOME 站姿的逐关节差达 20-106° —— 髋俯仰 -86°
    (HOME -26°)、膝 -86°(HOME 0°)、踝顶满 ±51° 限位(HOME ±26°)、neck_pitch
    -87°(HOME +20°)、head_yaw +105°、tail_pitch -47°、双臂 -33°/-65°。
    现有指标门量不出这个:蜷缩躯干的根高"碰巧"落进 103.5-125.5mm 窗,倾角也够,
    于是十二项判据里没有任何一项在惩罚姿势本身。这是八轮指标优化后"姿势仍是错的"
    的机制,也正是 squat_tracking(参考表 + 关节误差高斯)在蹲技能里防住的同一件事
    —— 蹲技能当时同样发现"不模仿参考的策略会找到别的满足高度的方式"。

    参考位形 = v7 HOME(contract 的 action_offset),由 cfg 注入;策略观测里本来
    就有 joint_pos,不存在特权输入。两层收益(与 squat_tracking 同型、与
    VelStand 的势函数经验一致):
    - pose:任务后半段(t>coach_after_s)按 exp(-((q-HOME)/std)²) 的关节均值,
      给"向正确站姿靠拢"的持续梯度;不限死时间窗内成立与否,早靠早得分;
    - hold:motion_stable_stand 的合取成立时的真站稳 —— 只有姿势对才可能同时
      满足非足支地+双脚受力+直立+静止,蜷缩姿态天然拿不到。
    前半段(起身翻滚)完全不罚,起身轨迹仍由进度项主导。
    """
    ...
    def __init__(self,cfg,env):
        ...
        self.dt=env.step_dt
        self.home=torch.tensor(cfg.params['home_pose'],device=env.device,dtype=torch.float32)
        self.pose_std=float(cfg.params.get('pose_std_rad',.25))
        self.hold_std=float(cfg.params.get('hold_std_rad',.35))
        self.after=float(cfg.params.get('coach_after_s',6.))
        self.hold_weight=float(cfg.params.get('hold_weight',6.))
        # hold×姿势门:hold 分只在姿势够近时才发。权12 后的实测(iter24600):教练分
        # ~2.5 里 85% 是 hold —— 蜷缩姿态把站稳条件榨满、白拿保底工资,而伸直腿的
        # 每一步要付 action_rate + hold 暂时中断的成本,净亏,于是 86° 的腿行程
        # 一步都不动(平均偏差 37°→37°,膝/脖/尾纹丝不动)。解耦考核的唯一办法是
        # 让"站稳的钱"也只在正确姿势上存在:蜷缩站立收入清零,直腿站立才有全额。
        self.hold_gate=float(cfg.params.get('hold_gate',.3))
        self.pose_free=bool(cfg.params.get('pose_free',True))
        self.pose_soft=float(cfg.params.get('pose_soft',.15))
        action=env.action_manager.get_term('joint_pos')
        self.ids=action._target_ids
        self.names=action._target_names

    def __call__(self,env,**params):
        robot=env.scene['robot']
        q=(robot.data.joint_pos+robot.data.encoder_bias)[:,self.ids]
        err=(q-self.home[None,:])/self.pose_std
        per=torch.exp(-(err*err))
        # 聚合器用 soft-min 而不是 mean。实测(iter26400 逐关节拆账):
        # head_pitch/head_roll/jaw/arm_l/arm_r 已 0.81-0.98 白送分,而
        # left_hip_pitch/knee/ankle/neck_pitch/head_yaw/right_hip_pitch/tail_pitch
        # 七个是 0.000;mean 让策略"继续磨那 15 个已经好的"就能撑住 0.29,
        # 永远不碰这七个。这就是聚合指标藏住绑定约束 —— 与今晚逐条 margin 的教训同源。
        # soft-min 让分数由最差关节决定,修膝/脖/尾直接见效;归一化保证全对时=1。
        n=per.shape[-1]
        pose=-self.pose_soft*torch.logsumexp(-per/self.pose_soft,dim=-1)+self.pose_soft*math.log(n)
        t=env.episode_length_buf*self.dt
        if self.pose_free:
            pose_w=torch.ones_like(t)
        else:
            pose_w=(t>self.after).float()
        pose=pose*pose_w
        with torch.no_grad():
            gravity=robot.data.projected_gravity_b
            support=env.scene['feet_ground_contact'].data.found.reshape(env.num_envs,-1).gt(0).all(-1)
            nonfoot=env.scene['nonfoot_ground'].data.found.reshape(env.num_envs,-1).gt(0).any(-1)
            z=robot.data.root_link_pos_w[:,2]-env.scene.env_origins[:,2]
            hold=torch.exp(-((z-.1135)/.012)**2-gravity[:,:2].square().sum(-1)/.04
                -robot.data.root_link_lin_vel_b.square().sum(-1)/.0025
                -robot.data.root_link_ang_vel_b.square().sum(-1)/.25)
            hold*=support*(~nonfoot)*(gravity[:,2]<-.94).float()
            hold*=torch.exp(-(((q-self.home[None,:])/self.hold_std)**2).mean(-1))
            # 姿势门:整段姿势分不达 hold_gate 时 hold 一分不给。
            hold*= (pose>self.hold_gate).float()
        # 坐撑示范环境(SitFoldResidualAction 分类)跳过:它们的动作被折腿时间表
        # 钉死,残差只有 ±0.015rad,pose/hold 分对策略不可动 —— 留着只会把不可控
        # 的误差灌进 advantage,稀释自由环境的学习信号。
        sit=getattr(env,'sitfold_script_mask',None)
        if sit is not None:
            pose=torch.where(sit,torch.zeros_like(pose),pose)
            hold=torch.where(sit,torch.zeros_like(hold),hold)
        return pose+self.hold_weight*hold


def motion_running_double_support_cost(env):
    """Remove a measured fast-walking plateau; nonnegative cost, no side bias."""
    feet=env.scene['feet_ground_contact'].data.found.reshape(env.num_envs,-1).gt(0).all(-1)
    robot=env.scene['robot']
    commanded=env.command_manager.get_command('twist')[:,0]>.4
    moving=robot.data.root_link_lin_vel_b[:,0]>.2
    return (feet & commanded & moving).float()


def feet_clearance_shortfall(env, target_height, height_sensor_name,
                             command_name=None, command_threshold=0.01,
                             asset_cfg=SceneEntityCfg(name='robot',
                                                      site_names=['left_foot', 'right_foot'])):
    """只罚"抬脚不足",不罚"抬得过高"(hinge)。上台阶专用。

    依据(2026-09-18 实测):mjlab 内置 feet_clearance 是**双向**偏离惩罚
    delta=|h-target|。把 target 从 0.02 提到 0.04 后,策略实际只能抬 12-15mm,
    偏离反而从 5-8mm 涨到 25-28mm,梯度混乱 —— 200 迭代后连原本能过的
    10mm 台阶都走不动(前进 3.0m -> 0.51m),且不摔倒只是不走。
    上台阶真正需要的是"抬够高度";抬过头只是费能量,已由 action_rate_l2 /
    feet_air_time(threshold_max=0.3s) 间接约束。所以正确形状是
    max(0, target - h):抬够了零成本,抬不够才付钱,梯度单向明确。
    """
    asset: Entity = env.scene[asset_cfg.name]
    ...
    shortfall = torch.clamp(target_height - foot_height, min=0.)  # 单向
    assert shortfall.shape == vel_norm.shape, (
        f'脚数不匹配: foot_height {tuple(shortfall.shape)} vs vel {tuple(vel_norm.shape)}; '
        f'asset_cfg.site_names 必须只含双脚')
    cost = torch.sum(shortfall * vel_norm, dim=1)                 # [B]
