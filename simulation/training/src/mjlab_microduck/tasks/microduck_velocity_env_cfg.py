...
from mjlab_microduck.action_filter import FilteredJointPositionActionCfg
...
def make_microduck_velocity_env_cfg(
    ...
    reset_z=(0.12, 0.13),
    pose_exclude_extra: str = "",
    rough_terrains=None,
    arm_pose_cmd: bool = False,
    tail_pose_cmd: bool = False,
    jaw_pose_cmd: bool = False,
    lin_vel_max: float | None = None,
    body_pose_weight: float = 0.0,
    body_nominal_height: float = 0.095,
    sol_iter: int | None = None,
    limb_track_scale: float = 1.0,
    servo_dr: dict | None = None,
    obs_delay: tuple[int, int] | None = None,
    stand_still_weight: float = 0.0,
    stand_slip_weight: float = 0.0,
    head_gaze_weight: float = 0.0,
    head_world_weight: float = 0.0,
    action_filter: tuple[float, float | None] | None = None,
    lateral_drift_weight: float = 0.0,
    stand_jitter_weight: float = 0.0,
    foot_condim6: bool = False,
    action_scale_dr: tuple[float, float] | None = None,
    ang_vel_max: float | None = None,
    ang_track_weight: float | None = None,
) -> ManagerBasedRlEnvCfg:
    """Create Microduck velocity tracking environment configuration.

    robot_cfg: the articulated robot (microduck by default; MicroDinosaur passes its
    own model).  reset_z: freejoint z range — the MicroDinosaur CAD trunk sits at
    0.117 m rather than the microduck 0.12 m.
    """
    ...
    # Action low-pass + slew-rate limit, applied during training (see
    # mjlab_microduck/action_filter.py for the measurements and the sources).
    # Placed AFTER the scale is set so the filtered values are in command units.
    if action_filter is not None:
        lp_alpha, max_delta = action_filter
        cfg.actions["joint_pos"] = FilteredJointPositionActionCfg(
    ...
    # pose_exclude_extra lets a robot add joints that must stay OUT of the pose
    # reward (MicroDinosaur: the balance tail is free to move for attitude
    # control, so pulling it to HOME would fight its purpose).
    cfg.rewards["pose"].params["asset_cfg"] = SceneEntityCfg(
        "robot", joint_names=(rf"^(?!passive_|.*neck.*|.*head.*{pose_exclude_extra}).*",)
    )
    ...
    cfg.rewards["track_angular_velocity"].weight = (
        ang_track_weight if ang_track_weight is not None else 2.0
    )
    ...
    # condim-6 feet (MicroDinosaur): MuJoCo only applies torsional/rolling
    # friction at condim>=4/6, so the single-parameter event above leaves those
    # at their nominal values.  Only the SLIDE axis is randomized.
    #
    # MEASURED 2026-09-13: randomizing torsion (or roll) is a runtime landmine in
    # MJX/warp -- the solver cost explodes with the SAMPLED value: 176 s/iter
    # with a large torsion draw, 194 s/iter at mu_roll=1e-3, against 3-5 s/iter
    # for the same condim=6 contacts at small values, while plain MuJoCo CPU is
    # indifferent (0.058 ms/step for condim 3/4/6).  Because the event is a
    # single shared draw, one unlucky sample silently makes a whole run 30-60x
    # slower.  The nominal (1.0, 0.01, 1e-6) from the constants stays -- that is
    # what the physics fix needs -- but it is not randomized.
    if foot_condim6:
        # Foot friction is FIXED at the nominal (1.0, 0.01, 1e-6) from the
        # constants -- no randomization at all.  MEASURED 2026-09-13: with
        # condim=6 feet, MJX/warp's solver cost depends on the SAMPLED friction
        # value -- the same code ran at 3-5 s/iter or 25-176 s/iter depending on
        # the draw (three runs lost to this: a torsion draw, a roll draw and a
        # slide draw), and GPU utilization drops to ~23% in the slow regime, i.e.
        # solver thrashing rather than compute.  Plain MuJoCo CPU is indifferent
        # (0.058 ms/step for condim 3/4/6).  The original single-axis event above
        # is dropped so the model's nominal friction is what runs; friction DR
        # gets reintroduced only after the safe envelope is mapped on this stack.
    ...
    cfg.events["reset_base"].params["pose_range"]["z"] = reset_z
    ...
    # Feedback (observation) delay, modelled SEPARATELY from the command delay
    # (which lives on the actuator cfg).  Per the 2026-09-12 hardware spec the
    # bus itself is sub-timestep at 500 Hz, so this covers the remaining
    # measurement chain; the range is a PLACEHOLDER until timestamps are logged.
    if obs_delay:
        for group in ("actor", "critic"):
            for term in ("base_ang_vel", "joint_pos", "joint_vel"):
                if term in cfg.observations[group].terms:
                    cfg.observations[group].terms[term].delay_min_lag = obs_delay[0]
                    cfg.observations[group].terms[term].delay_max_lag = obs_delay[1]
                    cfg.observations[group].terms[term].delay_per_env = True
                    cfg.observations[group].terms[term].delay_hold_prob = 0.5

    # Zero-command stillness: prices in-place rotation (yaw rate) and horizontal
    # drift only.  Pitch/roll rate and the 19 joint velocities are deliberately
    # NOT priced -- a standing biped needs them to balance, and pricing them made
    # the shared policy degrade globally (see stand_still_penalty docstring).
    if stand_still_weight:
        cfg.rewards["stand_still"] = RewardTermCfg(
            func=microduck_mdp.stand_still_penalty,
            weight=-stand_still_weight,
            params={
                "command_name": "twist",
    ...
    # Zero-command FOOT SLIP: mjlab's feet_slip only prices slip while the
    # command is non-zero, so a planted foot can grind for free at standstill
    # (measured: +14 deg/s in-place rotation with the feet down).  Same form,
    # gate inverted -- walking/turning envs are unaffected.
    if stand_slip_weight:
        cfg.rewards["stand_slip"] = RewardTermCfg(
            func=microduck_mdp.stand_slip_penalty,
            weight=-stand_slip_weight,
            params={
    ...
    # Persistent lateral LEAN (observable via projected gravity) -- the
    # sideways-wander driver.  Replaces the earlier |vy| penalties, which the
    # policy could not act on (base linear velocity is critic-only).
    if lateral_drift_weight:
        cfg.rewards["lean_drift"] = RewardTermCfg(
            func=microduck_mdp.lean_drift_penalty,
            weight=-lateral_drift_weight,
            params={"ema_alpha": 0.01},
        )

    # Left/right duty-cycle imbalance (one foot staying down longer).
    if lateral_drift_weight:
        cfg.rewards["contact_timing"] = RewardTermCfg(
            func=microduck_mdp.contact_timing_asymmetry,
            weight=-lateral_drift_weight,
            params={"sensor_name": "feet_ground_contact", "ema_alpha": 0.005,
                    "command_name": "twist"},
        )

    # Head held at a commanded WORLD orientation (stabilised camera platform).
    if head_world_weight:
        cfg.rewards["head_world_gaze"] = RewardTermCfg(
            func=microduck_mdp.head_world_gaze_tracking,
            weight=head_world_weight,
            params={
                "command_name": "head_pose",
                # 0.25 measured DEAD: the initial world-orientation error is far
                # outside the kernel, exp(-e^2/s^2) ~ 0 and the term produced no
                # gradient at all (smoke test: Episode_Reward/head_world_gaze =
                # 0.0000 for every iteration).  0.5 rad = 29 deg leaves a real
                # signal at the start and still sharpens to ~4 deg.
                "std": 0.5,
                "asset_cfg": SceneEntityCfg("robot", body_names=("jaw_soft",)),
            },
        )

    # Head-gaze stability (always on -- the head must be quiet while walking).
    if head_gaze_weight:
        cfg.rewards["head_gaze"] = RewardTermCfg(
            func=microduck_mdp.head_gaze_penalty,
            weight=-head_gaze_weight,
            params={"asset_cfg": SceneEntityCfg(
                "robot", body_names=("neck", "neck_pitch", "yaw_roll_motion"))},
        )

    # Zero-command joint fidget (small, gated; see stand_jitter_penalty).
    if stand_jitter_weight:
        cfg.rewards["stand_jitter"] = RewardTermCfg(
            func=microduck_mdp.stand_jitter_penalty,
            weight=-stand_jitter_weight,
            params={"command_name": "twist", "command_threshold": 0.05},
        )

    # Gait-amplitude randomization: one policy that stays coordinated across
    # stride sizes (see randomize_action_scale docstring).
    if action_scale_dr:
        cfg.events["randomize_action_scale"] = EventTermCfg(
            func=microduck_mdp.randomize_action_scale,
            mode="reset",
            params={"action_name": "joint_pos", "scale_range": action_scale_dr},
        )

    # Servo-parameter randomization (scale factors around the configured nominal
    # values).  Use this whenever the real servo's stiffness/damping/torque are
    # known only approximately: the policy must work across the plausible set,
    # not just at one guessed operating point.  Keys: kp, kd, effort.
    if servo_dr:
        if servo_dr.get("kp") or servo_dr.get("kd"):
            cfg.events["randomize_servo_gains"] = EventTermCfg(
                func=dr.pd_gains,
                mode="reset",
                params={
                    "asset_cfg": SceneEntityCfg("robot"),
                    "operation": "scale",
                    "kp_range": servo_dr.get("kp", (1.0, 1.0)),
                    "kd_range": servo_dr.get("kd", (1.0, 1.0)),
                },
            )
        if servo_dr.get("effort"):
            cfg.events["randomize_servo_effort"] = EventTermCfg(
                func=dr.effort_limits,
                mode="reset",
                params={
                    "asset_cfg": SceneEntityCfg("robot"),
                    "operation": "scale",
                    "effort_limit_range": servo_dr["effort"],
                },
            )
    ...
    command.ranges.lin_vel_x = (-lin_vel_max, lin_vel_max) if lin_vel_max else (-0.4, 0.4)
    command.ranges.lin_vel_y = (-0.3, 0.3)
    command.ranges.ang_vel_z = (-ang_vel_max, ang_vel_max) if ang_vel_max else (-1.0, 1.0)
    ...
    cfg.commands["body_pose"] = microduck_mdp.UniformPoseCommandCfg(
        ...
        ranges=(
            (-0.005, 0.005),  # x (m)
            (-0.005, 0.005),  # y (m)
            (-0.04, 0.04),    # z (m) — crouch/stand height, real command now
            (-0.15, 0.15),    # roll (rad) — body sway for dancing
            (-0.25, 0.25),    # pitch (rad) — lean / prone tendency
            (-0.05, 0.05),    # yaw (rad)
        ),
    )
    if arm_pose_cmd:
        # Arm gesture command: 2-D deltas from the arm defaults (arm_l, arm_r).
        # Range is the full usable sweep; the tracking reward below teaches
        # "gesture while balancing" in the same policy.
        cfg.commands["arm_pose"] = microduck_mdp.UniformPoseCommandCfg(
            resampling_time_range=(2.0, 5.0),
            ranges=((-1.0, 1.0), (-1.0, 1.0)),
        )
    if tail_pose_cmd:
        # Tail command: 2-D deltas (tail_yaw, tail_pitch). Wagging is a
        # deployment-side oscillation of this command — the policy only needs
        # to track the commanded angle while walking/balancing.
        #
        # Pitch range widened 2026-09-13 (user: "让尾巴完全竖起来…尾巴可以竖着
        # 走"): the old +-0.8 rad cap meant the policy was never even ASKED for a
        # raised tail, so no amount of training could produce one.  The tail is
        # nearly level at HOME (~0 deg) and vertical is ~+1.57 rad, so the range
        # now reaches +1.75 with margin; yaw stays +-0.6 (it only steers the
        # tail left/right).  Requires the matching joint limit (blend2mjcf
        # JOINT_LIMIT table) — both were +-0.9/+-0.8 before.
        cfg.commands["tail_pose"] = microduck_mdp.UniformPoseCommandCfg(
            resampling_time_range=(0.8, 1.6),
            ranges=((-0.6, 0.6), (-0.8, 1.75)),
        )
    if jaw_pose_cmd:
        # Jaw command: 1-D mouth opening delta (expressions / lip-sync).
        cfg.commands["jaw_pose"] = microduck_mdp.UniformPoseCommandCfg(
    ...
    for group in ("actor", "critic"):
        ...
        if arm_pose_cmd:
            # Arm gesture channel (MicroDinosaur forelimbs): same mechanism as
            # head_pose — a 2-D delta command tracked as a primary objective.
            cfg.observations[group].terms["arm_command"] = ObservationTermCfg(
                func=mdp.generated_commands,
                params={"command_name": "arm_pose"},
            )
        if tail_pose_cmd:
            cfg.observations[group].terms["tail_command"] = ObservationTermCfg(
                func=mdp.generated_commands,
                params={"command_name": "tail_pose"},
            )
        if jaw_pose_cmd:
            cfg.observations[group].terms["jaw_command"] = ObservationTermCfg(
                func=mdp.generated_commands,
                params={"command_name": "jaw_pose"},
            )
    ...
    # 手势跟踪课程的起始迭代: 步态先建立, 再把手势跟踪权重从 0 渐进加入, 否则
    # 策略会"站着跟踪手势"而不学走路(2026-09-12 诊断: 关掉手势跟踪后 air_time
    # 从 0.015 跳到 2.68, 仅 186 迭代)。
    LIMB_TRACK_RAMP_START = 800
    if limb_track_scale > 0 and not play:
        cfg.curriculum["arm_track_weight"] = CurriculumTermCfg(
            func=microduck_mdp.reward_weight,
            params={"reward_name": "arm_pose_tracking", "weight_stages": [
                {"step": 0, "weight": 0.0},
                {"step": LIMB_TRACK_RAMP_START * NUM_STEPS_PER_ENV, "weight": 0.5},
                {"step": (LIMB_TRACK_RAMP_START + 600) * NUM_STEPS_PER_ENV,
                 "weight": 1.5 * limb_track_scale},
            ]},
        )
        cfg.curriculum["tail_track_weight"] = CurriculumTermCfg(
            func=microduck_mdp.reward_weight,
            params={"reward_name": "tail_pose_tracking", "weight_stages": [
                {"step": 0, "weight": 0.0},
                {"step": LIMB_TRACK_RAMP_START * NUM_STEPS_PER_ENV, "weight": 0.5},
                {"step": (LIMB_TRACK_RAMP_START + 600) * NUM_STEPS_PER_ENV,
                 "weight": 1.5 * limb_track_scale},
        ...
        cfg.curriculum["jaw_track_weight"] = CurriculumTermCfg(
            func=microduck_mdp.reward_weight,
            params={"reward_name": "jaw_pose_tracking", "weight_stages": [
                {"step": 0, "weight": 0.0},
                {"step": LIMB_TRACK_RAMP_START * NUM_STEPS_PER_ENV, "weight": 0.3},
                {"step": (LIMB_TRACK_RAMP_START + 600) * NUM_STEPS_PER_ENV,
                 "weight": 1.0 * limb_track_scale},
            ]},
        )
    if arm_pose_cmd:
        cfg.rewards["arm_pose_tracking"] = RewardTermCfg(
            func=microduck_mdp.limb_pose_tracking,
            weight=1.5 * limb_track_scale,
            params={
                "command_name": "arm_pose",
                "joint_patterns": (r".*arm_l.*", r".*arm_r.*"),
                "std": 0.5,
            },
        )
    if tail_pose_cmd:
        cfg.rewards["tail_pose_tracking"] = RewardTermCfg(
            func=microduck_mdp.limb_pose_tracking,
            weight=1.5 * limb_track_scale,
            params={
                "command_name": "tail_pose",
                "joint_patterns": (r".*tail_yaw.*", r".*tail_pitch.*"),
                "std": 0.5,
            },
        )
    if jaw_pose_cmd:
        cfg.rewards["jaw_pose_tracking"] = RewardTermCfg(
            func=microduck_mdp.limb_pose_tracking,
            weight=1.0 * limb_track_scale,
            params={
                "command_name": "jaw_pose",
                "joint_patterns": (r".*jaw_hinge.*",),
                "std": 0.3,
            },
        )
    ...
    # command stay alive for envs that raise the weight (standup).  MicroDinosaur
    # tasks pass body_pose_weight > 0 to make height/roll/pitch real commands
    # (crouch / body sway / lean).
    cfg.rewards["body_pose_tracking"] = RewardTermCfg(
        func=microduck_mdp.body_pose_tracking_6d,
        weight=body_pose_weight,
        params={
            "command_name": "body_pose",
            "nominal_height": body_nominal_height,
            "xy_std": 0.05,
            "z_std": 0.02,
    ...
    if sol_iter is not None:
        cfg.sim.mujoco.iterations = sol_iter
        cfg.sim.mujoco.ls_iterations = 2 * sol_iter
    ...
    if not rough:
        ...
        cfg.scene.terrain.terrain_generator = rough_terrains or MICRODUCK_ROUGH_TERRAINS_CFG
