...
# roller_slope.
PLAY_FACE_UP = None
# Même principe pour les départs SUR LE CÔTÉ (les deux côtés réunis) :
#   STANDUP_PLAY_SIDE=1.0  -> 100 % de départs couchés sur un côté
#   STANDUP_PLAY_SIDE=0.2  -> le mélange du dernier palier du curriculum
#   non définie -> comportement par défaut. N'a d'effet qu'en play=True.
PLAY_SIDE = None
...
def _resolve_play_side():
    """Proportion de départs couchés sur le côté au play : env STANDUP_PLAY_SIDE sinon la constante."""
    raw = os.environ.get("STANDUP_PLAY_SIDE")
    if raw is None:
        return PLAY_SIDE
    raw = raw.strip().lower()
    if raw in ("", "none", "random"):
    ...
    try:
        ...
        print(f"[roller_standup] STANDUP_PLAY_SIDE='{raw}' invalide -> défaut {PLAY_SIDE}")
        return PLAY_SIDE
...
# Les constantes ci-dessus sont en ESPACE MODÈLE (18 joints, roues incluses).
# Les params ``joint_indices`` des récompenses de mdp.py doivent au contraire
# être exprimés en ESPACE 14 SERVOS (cf. docstring `_servo_joint_ids`) : sur le
# modèle rollers l'ordre servi est gauche(5) → cou(4) → droite(5). Les index
# 11-15 de _LEG_JOINTS déborderaient du tableau joint_pos 14-col → assert GPU.
_LEG_JOINTS_SERVO = [0, 1, 2, 3, 4, 9, 10, 11, 12, 13]
_NECK_JOINTS_SERVO = [5, 6, 7, 8]
...
def make_microduck_roller_standup_env_cfg(play: bool = False) -> ManagerBasedRlEnvCfg:
    ...
    cfg.rewards["pose_stand_legs"] = RewardTermCfg(
        ...
        params={
            "std": 0.5,
            "joint_indices": _LEG_JOINTS_SERVO,
    ...
    cfg.events["set_ground_state"] = EventTermCfg(
        ...
        params={
            ...
            "side_left_prob":  0.00,  # couchés sur le côté — introduits encore après
            "side_right_prob": 0.00,
            "sitting_prob":   0.00,
            "standing_prob":  0.50,
    ...
    # Curriculum des poses de départ, easy → hard, sur les QUATRE orientations
    # de chute : ventre, dos, côté gauche, côté droit (+ debout, pour tenir).
    # Avec un mélange plat dès le départ, la policy optimise la majorité facile
    # et laisse les poses dures sous-entraînées (leçon du standup : il gelait en
    # « ne rien faire » sur le dos). On introduit donc debout+ventre d'abord,
    # le dos ensuite, les CÔTÉS en dernier — un départ sur le côté demande de
    # rouler d'abord vers le ventre/le dos avant de se relever (compétence en
    # plus, pas juste une orientation de plus) — puis on biaise vers les poses
    # dures à la fin.
    _SIDE_TOTAL = 0.20  # part des deux côtés réunis à partir du palier 2
        ...
        params={
            ...
            "param_stages": [
                {"step": 0, "params": {
                    ...
                    "side_left_prob": 0.00, "side_right_prob": 0.00}},
                ...
                {"step": 1500 * NUM_STEPS_PER_ENV, "params": {
                    ...
                    "face_down_prob": 0.30, "face_up_prob": 0.25,
                    "side_left_prob": _SIDE_TOTAL / 2, "side_right_prob": _SIDE_TOTAL / 2}},
                {"step": 2500 * NUM_STEPS_PER_ENV, "params": {
                    "standing_prob": 0.20, "sitting_prob": 0.00,
                    "face_down_prob": 0.30, "face_up_prob": 0.30,
                    "side_left_prob": _SIDE_TOTAL / 2, "side_right_prob": _SIDE_TOTAL / 2}},
            ],
        },
    )

    # Override de play : forcer les départs sur le dos ou sur le côté pour
    # pouvoir les inspecter. On écrit les probabilités dans l'événement ET on
    # retire le curriculum : sans ça, event_param_curriculum (qui tourne AVANT
    # les événements de reset) les réécrirait avec son palier 0 dès le premier
    # reset. Uniquement en play, donc l'entraînement et son curriculum easy →
    # hard sont intouchés.
    if play:
        play_side = _resolve_play_side()
        if play_side is not None:
            remainder = 1.0 - play_side
            cfg.events["set_ground_state"].params.update({
                "side_left_prob":  play_side / 2,
                "side_right_prob": play_side / 2,
                "face_up_prob":    remainder * 0.5,
                "face_down_prob":  remainder * 0.5,
                "standing_prob":   0.00,
                "sitting_prob":    0.00,
            })
