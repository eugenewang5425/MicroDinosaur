...
try:
    ...
    import tty
    _HAS_POSIX_TTY = True
except ImportError:  # Windows: raw keyboard via msvcrt
...
class TerminalInput:
    ...
    # Windows msvcrt scancodes for arrow keys (getwch returns prefix + code)
    _WIN_ARROWS = {"H": "up", "P": "down", "K": "left", "M": "right"}

    def __init__(self):
        self._queue = queue.Queue()
        self.enabled = sys.stdin.isatty()
        self._fd = sys.stdin.fileno() if self.enabled and _HAS_POSIX_TTY else -1
    ...
    def __exit__(self, *exc):
        self._stop.set()
        if _HAS_POSIX_TTY and self._old_attrs is not None:
    ...
    def _read1(self, timeout):
        ...
        select reported ready. POSIX only."""
    ...
    def _reader_windows(self):
        while not self._stop.is_set():
            if not _msvcrt.kbhit():
                ...
                continue
            ch = _msvcrt.getwch()
            if ch in ("\x00", "\xe0"):  # special key: arrow scancode follows
                code = _msvcrt.getwch()
                name = self._WIN_ARROWS.get(code)
                if name:
                    self._queue.put(name)
                continue
            self._queue.put(ch.lower() if ch.isalpha() else ch)
...
def main():
    ...
    parser.add_argument("--headless", type=int, default=None,
                        help="Run N control steps without the viewer window (headless verification, runs at full speed)")
    ...
    class _HeadlessViewer:
        """无窗口占位 (--headless 模式): is_running 恒真, sync 空操作."""
    ...
    if args.headless is not None:
        viewer_ctx = nullcontext(_HeadlessViewer())
    else:
        viewer_ctx = mujoco.viewer.launch_passive(
            model, data, show_left_ui=False, show_right_ui=False)

    with TerminalInput() as term, viewer_ctx as viewer:
        viewer.sync()
        ...
        try:
            ...
            while viewer.is_running() and not quit_requested:
                ...
                if args.headless is not None and control_step_count >= args.headless:
                    quit_requested = True
                ...
                if sleep_time > 0 and args.headless is None:
    ...
    if args.headless is not None:
        # MuJoCo qpos 四元数顺序为 [w, x, y, z] — w 是第 0 个分量
        quat = data.qpos[qpos_adr + 3:qpos_adr + 7]
        tilt = 2 * math.degrees(math.acos(min(1.0, abs(float(quat[0])))))
        pos = data.qpos[qpos_adr:qpos_adr + 3]
        print(f"[headless] {args.headless} 步结束: trunk 倾斜 {tilt:.1f}°, "
              f"位置 ({pos[0]:+.2f}, {pos[1]:+.2f}, {pos[2]:.3f}) m")
